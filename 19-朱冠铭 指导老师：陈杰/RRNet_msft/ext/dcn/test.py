...
def check_zero_offset():
    ...
    dcn_v2 = DCNv2(inC, outC, (kH, kW),
                   ...
                   deformable_groups=deformable_groups).npu()
    ...
    input = torch.randn(N, inC, inH, inW).npu()
...
def check_gradient_dconv():
    ...
    input = torch.rand(N, inC, inH, inW).npu() * 0.01
    input.requires_grad = True

    offset = torch.randn(N, deformable_groups * 2 * kW * kH, inH, inW).npu() * 2
    # offset.data.zero_()
    # offset.data -= 0.5
    offset.requires_grad = True

    mask = torch.rand(N, deformable_groups * 1 * kW * kH, inH, inW).npu()
    # mask.data.zero_()
    mask.requires_grad = True
    mask = torch.sigmoid(mask)

    weight = torch.randn(outC, inC, kH, kW).npu()
    weight.requires_grad = True

    bias = torch.rand(outC).npu()
...
def check_pooling_zero_offset():
    ...
    input = torch.randn(2, 16, 64, 64).npu().zero_()
    ...
    dpooling = DCNv2Pooling(spatial_scale=1.0 / 4,
                            ...
                            group_size=1,
                            trans_std=0.0).npu()
    offset = torch.randn(20, 2, 7, 7).npu().zero_()
...
def check_gradient_dpooling():
    input = torch.randn(2, 3, 5, 5).npu() * 0.01
    N = 4
    batch_inds = torch.randint(2, (N, 1)).npu().float()
    x = torch.rand((N, 1)).npu().float() * 15
    y = torch.rand((N, 1)).npu().float() * 15
    w = torch.rand((N, 1)).npu().float() * 10
    h = torch.rand((N, 1)).npu().float() * 10
    rois = torch.cat((batch_inds, x, y, x + w, y + h), dim=1)
    offset = torch.randn(N, 2, 3, 3).npu()
    input.requires_grad = True
    offset.requires_grad = True
...
def example_dconv():
    input = torch.randn(2, 64, 128, 128).npu()
    # wrap all things (offset and mask) in DCN
    dcn = DCN(64, 64, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).npu()
...
def example_dpooling():
    input = torch.randn(2, 32, 64, 64).npu()
    batch_inds = torch.randint(2, (20, 1)).npu().float()
    x = torch.randint(256, (20, 1)).npu().float()
    y = torch.randint(256, (20, 1)).npu().float()
    w = torch.randint(64, (20, 1)).npu().float()
    h = torch.randint(64, (20, 1)).npu().float()
    rois = torch.cat((batch_inds, x, y, x + w, y + h), dim=1)
    offset = torch.randn(20, 2, 7, 7).npu()
    input.requires_grad = True
    offset.requires_grad = True
