...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, hw_shape):
        ...
        N = q.shape[2]
        attn = (q * self.scale) @ k.transpose(-2, -1)           # q k 点乘
        if self.rel_pos_spatial:                                # 如果使用了相对位置
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x, hw_shape):
        x_norm = self.norm1(x)
        x_block, hw_shape_new = self.attn(x_norm, hw_shape)   # pool attention

        if self.dim_mul_in_att and self.dim != self.dim_out:
            x = self.proj(x_norm)
        x_res, _ = attention_pool(     # x 到 最外层的残差边
            x, self.pool_skip, hw_shape, has_cls_embed=self.has_cls_embed
        )
        x = x_res + self.drop_path(x_block)  # 最后输出
        x_norm = self.norm2(x)
