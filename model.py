...
# 就是将28*28的维度映射到512维度而后再映射到512而已
...
class FcNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.input_dim) # 先将输入的维度拉好
...
# 卷积块，最后输出的形状是[batch_size,16*5*5]
...
### Moderate size of CNN for CIFAR-10 dataset 就是用在CIFAR10上的CNN神经网络
class ModerateCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.conv_layer(x) # 卷积
        x = x.view(x.size(0), -1) # 更改尺寸
        x = self.fc_layer(x) # 全连接
        return x

### Moderate size of CNN for CIFAR-10 dataset，这个跟上面不一样之处在于经过卷积层以后，调整特征向量的方式不一样(forward里面)
...
# 由于MNIST的尺寸和CIFAR不一样，所以全连接层里面的尺寸不一样
