...
def train(start_epoch=1):
    ...
    saver = tf.train.Saver(tf.global_variables(), max_to_keep=None) #保存全部的模型
    ...
    with tf.Session(config=config) as sess:
        ...
        print('load model...')
        load_model_path = '../src/Model/' + network_type + '/' + str(network_type) + '_model/' + 'model.ckpt'
        ...
        for epoch in range(start_epoch, start_epoch + number_of_epoch):
            ...
            with open('../src/Result/' + str(network_type) + '_accuracy.txt', 'a') as file:
                file.write(str(epoch) + ' ' + str(iter) + ' : ' + str(val_accuracy) + '\n')
            print('   %d: accuracy = %.1f%%' % (epoch, val_accuracy*100.0))

            model_dir = '../src/Model/' + network_type + '/' + str(epoch) + '/'
