...
def get_sobel(in_chan, out_chan):
    ...
    filter_x = filter_x.reshape((1, 1, 3, 3)) # shape (1,1,3,3)
    filter_x = np.repeat(filter_x, in_chan, axis=1)  # 增加列数
...
class MVSSNet(ResNet50):
    def __init__(self, nclass, aux=False, sobel=False, constrain=False, n_input=3, is_plus=False, **kwargs):
    ...
    def forward(self, x, epoch):
        ...
        x0 = torch.sigmoid(x0)  # 将数值归一化到01
        if self.is_plus:
            clf = self.convGem(x0, epoch)
            outputs.append(clf)
            return res1, x0, clf
...
def get_mvss(backbone='resnet50', pretrained_base=True, nclass=1, sobel=True, n_input=3, constrain=True, is_plus=False, **kwargs):
