...
import torchvision
import numpy as np
# from utils.Gem_pool import GeM
...
class ConvGem(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = self.GeM(x0)
        res = (1 - 0.8) * self.GeM(x) + 0.8 * x0
        return res


class MVSSNet(ResNet50):
    def __init__(self, nclass, aux=False, sobel=False, constrain=False, n_input=3, activation=torch.nn.Sigmoid(),
                 is_plus=False, **kwargs):
    ...
    def forward(self, x):
        ...
        if self.sobel:
            ...
            res1 = self.erb_trans_3(res1 + self.upsample_4(self.erb_db_4(run_sobel(self.sobel_x4, self.sobel_y4, c4))),
                                    relu=False)
...
def get_mvss(backbone='resnet50', pretrained_base=True, nclass=1, sobel=True, n_input=3, constrain=True, is_plus=False,
             **kwargs):
...
if __name__ == '__main__':
    ...
    model = get_mvss(sobel=True, n_input=3, constrain=True, is_plus=True)
