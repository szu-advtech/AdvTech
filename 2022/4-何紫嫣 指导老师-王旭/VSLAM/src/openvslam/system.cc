...
system::system(const std::shared_ptr<config>& cfg, const std::string& vocab_file_path, cv::FileStorage tsdfSettings)
    ...
    // float meank = 50;
    // float thresh = 2.0;
    mpPointCloudMapping = make_shared<PointCloudMapping>( tsdfSettings );

    // tracking module
    tracker_ = new tracking_module(cfg_, this, map_db_, bow_vocab_, bow_db_, mpPointCloudMapping);
    // mapping module
