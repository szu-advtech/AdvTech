...
tracking_module::tracking_module(const std::shared_ptr<config>& cfg, system* system, data::map_database* map_db,
                                 data::bow_vocabulary* bow_vocab, data::bow_database* bow_db, shared_ptr<PointCloudMapping> pPointCloud)
    : cfg_(cfg), camera_(cfg->camera_), system_(system), map_db_(map_db), bow_vocab_(bow_vocab), bow_db_(bow_db),
      ...
      frame_tracker_(camera_, 10), relocalizer_(bow_db_), pose_optimizer_(),mpPointCloudMapping( pPointCloud ),
...
Mat44_t tracking_module::track_monocular_image(const cv::Mat& img, const double timestamp, const cv::Mat& mask) {
    ...
    // color conversion
    img_color_ = img;
    // cv::resize(img, img_color_, cvSize(2048, 1024), 0, 0, cv::INTER_LINEAR);
    cv::resize(img, img_gray_, cvSize(1024, 512));
    // img_gray_ = img;
...
void tracking_module::track() {
    ...
    if (tracking_state_ == tracker_state_t::Initializing) {
        ...
        if (succeeded && new_keyframe_is_needed()) {
            ...
            if ((kf_cnt % 3)==0)
            {
                vector<data::keyframe*> vpKFs = map_db_->get_all_keyframes();
                mpPointCloudMapping->insertKeyFrame( curr_frm_.ref_keyfrm_, this->img_color_, idk,vpKFs);
            }
            kf_cnt++;
            // vector<data::keyframe*> vpKFs = map_db_->get_all_keyframes();
            // mpPointCloudMapping->insertKeyFrame( curr_frm_.ref_keyfrm_, this->img_color_, idk,vpKFs);
...
void tracking_module::insert_new_keyframe() {
    ...
    // vector<data::keyframe*> vpKFs = map_db_->get_all_keyframes();
    // mpPointCloudMapping->insertKeyFrame( ref_keyfrm, this->img_color_, idk,vpKFs);
