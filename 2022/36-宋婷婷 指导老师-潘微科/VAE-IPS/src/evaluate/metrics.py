"""Evaluation metrics for Collaborative Filtering with Implicit Feedback."""
...
def dcg_at_k(y_true: np.ndarray, y_score: np.ndarray,
    ...
    y_true_sorted_by_score = y_true[y_score.argsort()[::-1]]    # argsort(): 返回元素从小到大排序后的索引数组; [::-1]表示倒序排序
    ...
    if not np.sum(y_true_sorted_by_score) == 0:
        for i in np.arange(k):
            if y_true_sorted_by_score[i] == 1:
                average_precision_score += np.sum(y_true_sorted_by_score[:i + 1] / pscore_sorted_by_score[:i + 1]) / (i + 1)
