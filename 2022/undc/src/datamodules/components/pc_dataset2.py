...
class ABC_pc_hdf5(torch.utils.data.Dataset):
    def __init__(
        ...
        with open(f"{data_dir}/my_obj_list.txt", "r") as fp:
            self.hdf5_names = [name.strip() for name in fp.readlines() if "s00" in name and 'trashcan' not in name]
    ...
    def __getitem__(self, index):
        ...
        # print(grid_size)
        # print(f"pc_KNN_idx : {pc_KNN_idx.shape}")  #[num_points * knn_num,]
        # print(f"pc_KNN_xyz : {pc_KNN_xyz.shape}")  #[num_points * knn_num,3]
...
class ABC_npc_hdf5(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print(pc_KNN_idx.shape)
        print(pc_KNN_xyz.shape)
        print(voxel_xyz_int.shape)
        print(voxel_KNN_idx.shape)
        print(voxel_KNN_xyz.shape)
        print(gt_output_bool.shape)
        print(gt_output_float.shape)
        print(gt_output_float_mask.shape)
        exit(0)
