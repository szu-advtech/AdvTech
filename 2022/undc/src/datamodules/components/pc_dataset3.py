...
MOTION_PATH = "/home/fubao/dev/InterRecon/motion"
...
class ABC_pc_hdf5(torch.utils.data.Dataset):
    def __init__(
        ...
        for cate in CATE_LIST:
            h5_file = h5py.File(f"{MOTION_PATH}/{cate}_motion.h5", 'r')
        ...
        with open(f"{data_dir}/my_obj_list.txt", "r") as fp:
            self.hdf5_names = [name.strip() for name in fp.readlines() if "s00" in name]
        ...
        if self.train:
            train_len = int(len(self.hdf5_names)*0.8)
            self.hdf5_names = self.hdf5_names[:train_len]
            self.pcs_txt_path_list = self.pcs_txt_path_list[:train_len]
            # ******* why
            # separate 32 and 64
            temp_hdf5_names = []
            temp_pcs_paths = []
