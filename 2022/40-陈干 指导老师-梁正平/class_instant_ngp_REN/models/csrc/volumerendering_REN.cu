...
__global__ void REN_composite_train_fw_kernel(
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> betas,
    ...
    torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> beta,
...
std::vector<torch::Tensor> REN_composite_train_fw_cu(
    ...
    auto beta = torch::zeros({N_rays}, sigmas.options());
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "REN_composite_train_fw_cu",
    ([&] {
        REN_composite_train_fw_kernel<scalar_t><<<blocks, threads>>>(
            betas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            ...
            T_threshold,
            beta.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
...
template <typename scalar_t>
__global__ void REN_composite_train_bw_kernel(
    ...
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> dL_dbeta,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> sigmas,
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> rgbs,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> betas,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> deltas,
    ...
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> beta,
    const scalar_t T_threshold,
    ...
    torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> dL_dbetas
){
    const int n = blockIdx.x * blockDim.x + threadIdx.x;
    if (n >= opacity.size(0)) return;
    ...
    // front to back compositing
    int samples = 0;
    scalar_t R = rgb[ray_idx][0], G = rgb[ray_idx][1], B = rgb[ray_idx][2], BETA_A = beta[ray_idx];
    scalar_t O = opacity[ray_idx], D = depth[ray_idx], Dsq = depth_sq[ray_idx];
    scalar_t T = 1.0f, r = 0.0f, g = 0.0f, b = 0.0f, d = 0.0f, dsq = 0.0f, BETA_a = 0.0f;
    ...
    while (samples < N_samples) {
        ...
        dL_dbetas[s] = dL_dbeta[ray_idx]*w;
        ...
        dL_dsigmas[s] = deltas[s] * (
            ...
            dL_dbeta[ray_idx]*(betas[s]*T-(BETA_A-BETA_a)) +
...
std::vector<torch::Tensor> REN_composite_train_bw_cu(
    ...
    auto dL_dbetas = torch::zeros({N}, sigmas.options());

    const int threads = 256, blocks = (N_rays+threads-1)/threads;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "REN_composite_train_bw_cu",
    ([&] {
        REN_composite_train_bw_kernel<scalar_t><<<blocks, threads>>>(
            ...
            dL_dbeta.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            sigmas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            rgbs.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            betas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            deltas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            ...
            dL_dbetas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>()
        );
    }));

    return {dL_dsigmas, dL_drgbs,dL_dbetas};
}
...
template <typename scalar_t>
__global__ void REN_composite_test_fw_kernel(
    ...
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> betas,
...
void REN_composite_test_fw_cu(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "REN_composite_test_fw_cu",
    ([&] {
        REN_composite_test_fw_kernel<scalar_t><<<blocks, threads>>>(
            ...
            betas.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
...
template <typename scalar_t>
__global__ void REN_composite_train_depth_fw_kernel(
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> sigmas,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> deltas,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> ts,
    const torch::PackedTensorAccessor64<int64_t, 2, torch::RestrictPtrTraits> rays_a,
...
std::vector<torch::Tensor> REN_composite_train_depth_fw_cu(
    const torch::Tensor sigmas,
    const torch::Tensor deltas,
    const torch::Tensor ts,
    const torch::Tensor rays_a,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "REN_composite_train_depth_fw_cu",
    ([&] {
        REN_composite_train_depth_fw_kernel<scalar_t><<<blocks, threads>>>(
            sigmas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            deltas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            ts.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            rays_a.packed_accessor64<int64_t, 2, torch::RestrictPtrTraits>(),
...
template <typename scalar_t>
__global__ void REN_composite_train_depth_bw_kernel(
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> dL_dopacity,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> dL_ddepth,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> dL_ddepth_sq,
    const torch::PackedTensorAccessor<scalar_t, 1, torch::RestrictPtrTraits, size_t> sigmas,
    ...
    scalar_t O = opacity[ray_idx], D = depth[ray_idx], Dsq = depth_sq[ray_idx];
    scalar_t T = 1.0f, d = 0.0f, dsq = 0.0f;
...
std::vector<torch::Tensor> REN_composite_train_depth_bw_cu(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_ddepth_sq,
    const torch::Tensor sigmas,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "REN_composite_train_depth_bw_cu",
    ([&] {
        REN_composite_train_depth_bw_kernel<scalar_t><<<blocks, threads>>>(
            dL_dopacity.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            dL_ddepth.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            dL_ddepth_sq.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
            sigmas.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>(),
