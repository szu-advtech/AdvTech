...
void composite_test_fw(
    ...
    composite_test_fw_cu(
        ...
        opacity_threshold, N_eff_samples,
        opacity, depth, rgb);
}



std::vector<torch::Tensor> REN_composite_train_fw(
    ...
    return REN_composite_train_fw_cu(
                betas,sigmas, rgbs, deltas, ts,
                rays_a, opacity_threshold);
}


std::vector<torch::Tensor> REN_composite_train_bw(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_ddepth_sq,
    const torch::Tensor dL_drgb,
    ...
    return REN_composite_train_bw_cu(
                dL_dopacity, dL_ddepth, dL_ddepth_sq, dL_drgb,dL_dbeta,
                sigmas, rgbs,betas, deltas, ts, rays_a,
                opacity, depth, depth_sq, rgb,beta, opacity_threshold);
}


void REN_composite_test_fw(
    const torch::Tensor sigmas,
    const torch::Tensor rgbs,
    const torch::Tensor betas,
    const torch::Tensor deltas,
    ...
    REN_composite_test_fw_cu(
        sigmas, rgbs,betas, deltas, ts, hits_t, alive_indices,
        opacity_threshold, N_eff_samples,
        opacity, depth, rgb,beta);
}



std::vector<torch::Tensor> REN_composite_train_depth_fw(
    const torch::Tensor sigmas,
    const torch::Tensor deltas,
    const torch::Tensor ts,
    const torch::Tensor rays_a,
    ...
    return REN_composite_train_depth_fw_cu(
                sigmas, deltas, ts,
                rays_a, opacity_threshold);
}


std::vector<torch::Tensor> REN_composite_train_depth_bw(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_ddepth_sq,
    const torch::Tensor sigmas,
    ...
    return REN_composite_train_depth_bw_cu(
                dL_dopacity, dL_ddepth, dL_ddepth_sq,
                sigmas, deltas, ts, rays_a,
                opacity, depth, depth_sq, opacity_threshold);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("REN_composite_train_fw", &REN_composite_train_fw);
    m.def("REN_composite_train_bw", &REN_composite_train_bw);
    m.def("REN_composite_test_fw", &REN_composite_test_fw);

    m.def("REN_composite_train_depth_fw", &REN_composite_train_depth_fw);
    m.def("REN_composite_train_depth_bw", &REN_composite_train_depth_bw);
}