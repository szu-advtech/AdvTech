...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        #设立样本以及推理出来的最大句子长度，默认是1024
        if getattr(args, "max_source_positions", None) is None:
            args.max_source_positions = DEFAULT_MAX_SOURCE_POSITIONS
        if getattr(args, "max_target_positions", None) is None:
            args.max_target_positions = DEFAULT_MAX_TARGET_POSITIONS

        #在OFA的任务设置当中，src_dict跟tgt_dict是一样的
        src_dict, tgt_dict = task.source_dictionary, task.target_dictionary

        #share_all_embedding指encoder跟decoder的投影层使用相同参数
        ...
        #如果有启用prompt/adapter，就对模型的参数进行冻结
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        #确定模型内的特征维度，paddingtoken的id，最大源句子长度以及注意力头数
        ...
        #图像投影层，把resnet的输出维度转换为transformer层的输入维度
        self.image_proj = Linear(1024, embed_dim)

        #读取图像embedding层，和其之后的layernorm层的参数
        ...
        #文本和图像embedding层的位置向量，都由可学习的embedding层提供
        #文本的位置映射层：输入维度是最大句子长度+2（bos and eos),输出维度是特征维度
        self.embed_positions = Embedding(args.max_source_positions + 2, embed_dim)
        #图像的位置映射层,输入维度是图像块数+1，输出是特征维度
        ...
        #这里其实经典transformer中的缩放因子，每个头的根号d_embed
        ...
        #drop_path：若x为输入的张量，其通道为[B,C,H,W]，那么drop_path的含义为在一个Batch_size中，随机有drop_prob的样本，不经过主干，而直接进行恒等映射。
        dpr = [x.item() for x in torch.linspace(0, args.encoder_drop_path_rate, args.encoder_layers)]

        #通过build_encoder_layer构建每一层
        self.layers.extend(
