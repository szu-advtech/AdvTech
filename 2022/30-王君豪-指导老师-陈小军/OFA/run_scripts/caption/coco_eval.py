...
if __name__ == "__main__":
    print(sys.argv)#默认的只有本py文件
    if len(sys.argv) == 3:
        evaluate_on_coco_caption(sys.argv[1], sys.argv[2])
    elif len(sys.argv) == 4:
        evaluate_on_coco_caption(sys.argv[1], sys.argv[2], sys.argv[3])
    elif len(sys.argv) == 1:
        res_file="../../results/caption/test_predict.json"
        label_file="../../dataset/caption_data/test_caption_coco_format.json"
        evaluate_on_coco_caption(res_file,label_file,outfile='outfile.json')
    else:
