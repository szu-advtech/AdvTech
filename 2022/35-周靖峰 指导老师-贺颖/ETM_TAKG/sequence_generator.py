...
class SequenceGenerator(object):
    ...
    def beam_search(self, src, src_lens, src_oov, src_mask, src_bow, oov_lists, word2idx, max_eos_per_output_seq=1):
        ...
        if self.use_topic_represent:
            ...
            self.ntm_model.eval()
            if self.ntm_type == 'ntm':
                if self.topic_type == 'z':
                    topic_represent, _, recon_batch, mu, logvar = self.ntm_model(src_bow_norm)
                else:
                    _, topic_represent, recon_batch, mu, logvar = self.ntm_model(src_bow_norm)
            elif self.ntm_type == 'etm':
                topic_represent, recon_loss, kld_theta = self.ntm_model(src_bow ,src_bow_norm)
