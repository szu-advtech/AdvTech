...
def train_ntm_one_epoch(model, dataloader, optimizer, opt, epoch, kl_weight):
    model.train()
    train_loss = 0
    for batch_idx, data_bow in enumerate(dataloader):
        ...
        elif opt.ntm_type == 'etm':
            theta, recon_loss, kld_theta = model(data_bow ,data_bow_norm)
            loss = recon_loss + kld_theta * kl_weight[(epoch - 1) * len(dataloader) + batch_idx]
        loss.backward()
        train_loss += loss.item()
        optimizer.step()
        if batch_idx % 100 == 0:
            # print("recon_loss: ", recon_loss.item())
            # print("kld_theta: ", kld_theta.item() * kl_weight[(epoch - 1) * len(dataloader) + batch_idx].item())
            # print("kl_weight", kl_weight[(epoch - 1) * len(dataloader) + batch_idx].item())
    ...
    if opt.ntm_type == 'ntm':
        ...
        sparsity = check_sparsity(model.alphas.weight.data)
...
def test_ntm_one_epoch(model, dataloader, opt, epoch, kl_weight):
    model.eval()
    test_loss = 0
    with torch.no_grad():
        for i, data_bow in enumerate(dataloader):
            ...
            if opt.ntm_type == 'ntm':
                ...
                theta, recon_loss, kld_theta = model(data_bow ,data_bow_norm)
                test_loss += recon_loss + kld_theta
...
def train_model(model, ntm_model, optimizer_ml, optimizer_ntm, optimizer_whole, train_data_loader, valid_data_loader,
    ...
    if opt.only_train_ntm or (opt.use_topic_represent and not opt.load_pretrain_ntm):
        ...
        kl_weight = frange_cycle_linear(len(train_bow_loader) * opt.ntm_warm_up_epochs, start = opt.klw_start, stop = opt.klw_stop, n_cycle = opt.klw_cycle, ratio = opt.klw_ratio)
        for epoch in range(1, opt.ntm_warm_up_epochs + 1):
            sparsity = train_ntm_one_epoch(ntm_model, train_bow_loader, optimizer_ntm, opt, epoch, kl_weight)
            val_loss = test_ntm_one_epoch(ntm_model, valid_bow_loader, opt, epoch, kl_weight)
            if epoch % 10 == 0:
...
def train_one_batch(batch, model, ntm_model, optimizer, opt, batch_i):
    ...
    if opt.use_topic_represent:
        ...
        if opt.ntm_type == 'ntm':
            ...
            topic_represent, recon_loss, kld_theta = ntm_model(src_bow ,src_bow_norm)
            if opt.add_two_loss:
                ntm_loss = recon_loss + kld_theta
