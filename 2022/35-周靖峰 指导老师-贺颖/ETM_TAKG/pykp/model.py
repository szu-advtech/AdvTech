...
class ETM(nn.Module):
    def __init__(self, opt, hidden_dim=500, l1_strength=0.001) -> None:
        super().__init__()
        self.topic_num = opt.topic_num
        self.vocab_size = opt.bow_vocab
        self.rho_size = opt.rho_size
        self.enc_drop = opt.enc_drop
        self.t_drop = nn.Dropout(self.enc_drop)
        self.l1_strength = torch.FloatTensor([l1_strength]).to(opt.device)
        self.theta_act = self.get_activation(opt.theta_act)
        
        self.rho = nn.Linear(self.rho_size, self.vocab_size, bias=False)
        
        self.alphas = nn.Linear(self.rho_size, self.topic_num, bias=False)
        ...
        self.mu_q_theta = nn.Linear(hidden_dim, self.topic_num, bias=True)
        self.logsigma_q_theta = nn.Linear(hidden_dim, self.topic_num, bias=True)
    ...
    def get_theta(self, normalized_bows):
        ...
        self.logsigma_theta = logsigma_theta
    ...
    def forward(self, x, x_norm):
        theta, kld_theta = self.get_theta(x_norm)
        beta = self.get_beta()
        preds = self.decode(theta, beta)
        recon_loss = -(preds * x).sum(1)
        recon_loss = recon_loss.mean()
        return theta, recon_loss, kld_theta
    ...
    def print_topic_words(self, vocab_dic, fn, n_top_words=10):
        with torch.no_grad():
            ...
            for k in range(self.topic_num):
                gamma = gammas[k]
                top_words = list(gamma.cpu().numpy().argsort()[-self.topic_num+1:][:-n_top_words - 1:-1])
                topic_words = [vocab_dic[a].strip() for a in top_words]
