...
class MViT(nn.Module):
    ...
    def forward(self, x):
        print(x.shape)
        x, bchw = self.patch_embed(x)  # 打成patch  输出(1*3136*96)

        H, W = bchw[-2], bchw[-1]
        B, N, C = x.shape
        print(B,N,C)
        if self.cls_embed_on:       # 训练时的类别embed
            cls_tokens = self.cls_token.expand(B, -1, -1)
            x = torch.cat((cls_tokens, x), dim=1)
        print('dd')
        print(x.shape)
        if self.use_abs_pos:         # 使用绝对位置
            x = x + self.pos_embed

        thw = [H, W]
        print(thw)
        for blk in self.blocks:
            x, thw = blk(x, thw)   # attention_block
            print(thw)

        x = self.norm(x)           # 线性层

        if self.cls_embed_on:       #如果使用了类别embed，就取第一位
            x = x[:, 0]
        else:
            x = x.mean(1)           # 做平均
