...
def plot(model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    ...
    print("plotttttttttttttttttttttttttttttttt")
    ...
    depth = jt.ones(batch_size * num_samples).float() * max_depth
    print("rend_util.get_depth(points, pose).reshape(-1).shape: ", rend_util.get_depth(points, pose).reshape(-1).shape, "network_object_mask.shape: ", network_object_mask.shape)
    ...
    # print("rend_util.get_depth(points, pose).reshape(-1): ", rend_util.get_depth(points, pose).reshape(-1)[0], "network_object_mask: ", network_object_mask[0])
    print("depth", depth[0])
    print("tyep(depth) : ", type(depth))
    
    print("network_object_mask[0]: ", network_object_mask[0])
    print("tyep(network_object_mask) : ", type(network_object_mask))
    
    print("dtype(rend_util.get_depth(points, pose).reshape(-1)): ",type(rend_util.get_depth(points, pose).reshape(-1)))
    print("rend_util.get_depth(points, pose).: ",rend_util.get_depth(points, pose).shape)
    print("rend_util.get_depth(points, pose)[:, 0, :]: ",rend_util.get_depth(points, pose)[:, 0, :])
    
    print("rend_util.get_depth(points, pose).reshape(-1): ",rend_util.get_depth(points, pose).reshape(-1)[0])
...
def get_surface_high_res_mesh(sdf, resolution=100):
    ...
    recon_pc = jt.from_numpy(recon_pc).float()
    ...
    if jt.det(vecs) < 0:
        vecs = jt.mm(jt.tensor([[1, 0, 0], [0, 0, 1], [0, 1, 0]]).float(), vecs)
    ...
    if (not (np.min(z) > 0 or np.max(z) < 0)):
        ...
        verts = jt.from_numpy(verts).float()
