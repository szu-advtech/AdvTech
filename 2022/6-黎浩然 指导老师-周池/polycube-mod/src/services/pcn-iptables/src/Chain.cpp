...
void Chain::updateChain() {
  ...
  if ((name == ChainNameEnum::INPUT) && (parent_.horus_enabled)) {
    ...
    if ((getRuleList().size() >= HorusConst::MIN_RULE_SIZE_FOR_HORUS) &&
      ...
      // record when starting to identify horus
      auto horus_cal_start = std::chrono::high_resolution_clock::now();

      horusFromRulesToMap(horus, getRuleList());

      // record when finished to identify horus and log it
      auto horus_cal_end = std::chrono::high_resolution_clock::now();
      std::chrono::duration<double> horus_cal_elapsed_seconds = horus_cal_end - horus_cal_start;
      logger()->info("Horus have been updated in {0} s!", horus_cal_elapsed_seconds.count());

      // if horus.size() >= MIN_RULES_HORUS_OPTIMIZATION
      // record when starting to insert horus
      auto horus_ins_start = std::chrono::high_resolution_clock::now();
      ...
      // record when finished to insert rules into horus and log it
      auto horus_ins_end = std::chrono::high_resolution_clock::now();
      std::chrono::duration<double> horus_ins_elapsed_seconds = horus_ins_end - horus_ins_start;
      logger()->info("Horus have been inserted in {0} s!", horus_ins_elapsed_seconds.count());   
    }
  }
  ...
  auto bv_com_start = std::chrono::high_resolution_clock::now();
  ...
  auto bv_com_end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> bv_com_elapsed_seconds = bv_com_end - bv_com_start;
  logger()->info("Bitvectors have been computed in {0} s!", bv_com_elapsed_seconds.count());   
  ...
  bool second = false;
  size_t field_sizes[ModulesConstants::NUM_FIELD] = {
    conntrack_map.size(),
    ipsrc_map.size(),
    ipdst_map.size(),
    protocol_map.size(),
  ...
  u_int8_t field_order[ModulesConstants::NUM_FIELD] = {
    ModulesConstants::CONNTRACKMATCH,
    ModulesConstants::IPSOURCE,
    ModulesConstants::IPDESTINATION,
    ModulesConstants::L4PROTO,
    ModulesConstants::PORTSOURCE,
    ModulesConstants::PORTDESTINATION,
    ModulesConstants::INTERFACE,
    ModulesConstants::TCPFLAGS 
  };

  for (int i = ModulesConstants::NUM_FIELD - 1; i > 0; i--) {
    for(int j = 0; j < i; j++) {
      if (field_sizes[field_order[j]] < field_sizes[field_order[j+1]]) {
        uint8_t tmp = field_order[j];
        field_order[j] = field_order[j+1];
        field_order[j+1] = tmp;
      }
    }
  }
  ...
  for (int j = 0; j < 2; j++) {
    if (j == 1)
      second = true;
    for (int k = 0; k < ModulesConstants::NUM_FIELD; k++) {
    // Looping through conntrack
    if (field_order[k]==ModulesConstants::CONNTRACKMATCH && !conntrack_map.empty() && conntrack_break ^ second) {
    ...
    if (field_order[k]==ModulesConstants::IPSOURCE && !ipsrc_map.empty() && (ipsrc_break ^ second)) {
    ...
    if (field_order[k]==ModulesConstants::IPDESTINATION && !ipdst_map.empty() && ipdst_break ^ second) {
    ...
    if (field_order[k]==ModulesConstants::L4PROTO && !protocol_map.empty() && protocol_break ^ second) {
    ...
    if (field_order[k]==ModulesConstants::PORTSOURCE && !portsrc_map.empty() && portsrc_break ^ second) {
    ...
    if (field_order[k]==ModulesConstants::PORTDESTINATION && !portdst_map.empty() && portdst_break ^ second) {
      // At least one rule requires a matching on source ports,
      // so inject the module  on the first available position
      newProgramsChain.insert(
          std::pair<std::pair<uint8_t, ChainNameEnum>, Iptables::Program *>(
    ...
    if (field_order[k]==ModulesConstants::INTERFACE && !interface_map.empty() && interface_break ^ second) {
    ...
    if (field_order[k]==ModulesConstants::TCPFLAGS && !flags_map.empty() && flags_break ^ second) {
  ...
  auto chain_created_and_loaded_end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> chain_created_and_loaded_elapsed_seconds = chain_created_and_loaded_end - bv_com_end;
  logger()->info("Chain have been created and loaded in {0} s!", chain_created_and_loaded_elapsed_seconds.count());   
  ...
  auto old_hain_removed_end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> old_hain_removed_elapsed_seconds = old_hain_removed_end - chain_created_and_loaded_end;
  logger()->info("Old chain have been removed in {0} s!", old_hain_removed_elapsed_seconds.count());   
