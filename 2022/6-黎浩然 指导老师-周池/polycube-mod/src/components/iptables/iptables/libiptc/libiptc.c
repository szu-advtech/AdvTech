...
// rule entry from which table it comes
#define TABLE_UNKNOWN	0x0000U
#define TABLE_RAW		0x0001U
#define TABLE_MANGLE	0x0002U
#define TABLE_NAT		0x0003U
#define TABLE_FILTER	0x0004U

#define DEBUG_BUFSIZE 	BUFSIZ
...
/*
 * name was changed from print pcn_rule to save_n_print_pcn_rule cause
 * the function is responsible for saving the rule as a matter of fact
 */
static void save_n_print_pcn_rule(const IPT_CHAINLABEL chain, const char *action, const struct ipt_entry *rule, unsigned int rulenum, struct xtc_handle *handle){
	...
	// DEBUG: buffer used to get stdout
	// char debug_buffer[DEBUG_BUFSIZE];
	
	strcpy(APPEND, "append");
	...
	append_match = strncmp(APPEND, action, 6);
	...
	input_match = strncmp(INPUT, chain, 5);
	output_match = strncmp(OUTPUT, chain, 6);

	// DEBUG: redirect  stdout to buffer
	// fflush(stdout);
	// setbuf(stdout, debug_buffer);

	if (postrouting_match == 0) {
		// postrouting is not support yet!
		return;
	} else if (prerouting_match == 0) {
		if (replace_match == 0 || insert_match == 0) {
			if (rule != NULL) {
				switch (rule->comefrom) {
				case TABLE_RAW: 
					if (replace_match == 0) {
						printf("polycubectl pcn-iptables chain INPUT rule del %d; ", rulenum);
						printf("polycubectl pcn-iptables chain INPUT insert id=%d ", rulenum);
					} else if (prerouting_match == 0) {
						printf("polycubectl pcn-iptables chain INPUT insert id=%d ", rulenum);
					}
					break;
				case TABLE_MANGLE: 
					printf("polycubectl pcn-iptables mangle %s %s id=%d", chain, action, rulenum);
					return;
				case TABLE_NAT: 
				...
				// replace or insert without rule is not permitted
				return;
			}
		} else if (delete_match == 0) {
			if (rule == NULL) {
				if (rulenum == -1) {
					printf("polycubectl pcn-iptables chain INPUT rule del %d", rulenum + 1);
				} 
				return;
			} else {
				switch (rule->comefrom) {
				case TABLE_RAW: 
					printf("polycubectl pcn-iptables chain INPUT rule del ");
					if (rulenum != -1) {
						printf("%d ", rulenum + 1);
						return;
					}
					break;
				case TABLE_MANGLE: 
					printf("polycubectl pcn-iptables mangle %s rule del", chain);
					break;
				case TABLE_NAT: 
		...
		} else if (append_match == 0 || flush_match == 0) { 
			if (rule == NULL) {
				printf("polycubectl pcn-iptables chain INPUT %s ", action);
				if (rulenum != -1) {
					printf("%d ", rulenum + 1);
				}	
				return;
				...
				case TABLE_RAW: 
					...
					break;
				case TABLE_MANGLE: 
					printf("polycubectl pcn-iptables mangle %s %s", chain, action);
					break;
				case TABLE_NAT: 
		...
		// DEBUG; example: 
		// `pcn-iptables -t raw -I PREROUTING -s 10.0.0.1 -j DROP` gives
		// `polycubectl pcn-iptables nat PREROUTING insert id=0 src=10.0.0.1/32 action=DROP`
		// fprintf(stderr, debug_buffer);
		// fprintf(stderr, "\n");
		// exit(1);
	...
	} else
	{
		...
		if (replace_match == 0 || insert_match == 0) {
			if (rule != NULL) {
				...
				case TABLE_RAW: 
					if (replace_match == 0) {
						printf("polycubectl pcn-iptables chain OUTPUT rule del %d; ", rulenum);
						printf("polycubectl pcn-iptables chain OUTPUT insert id=%d ", rulenum);
					} else {
						printf("polycubectl pcn-iptables chain OUTPUT insert id=%d ", rulenum);
					}
					break;
				...
				case TABLE_FILTER: 
					if (replace_match == 0) {
						printf("polycubectl pcn-iptables chain %s rule del %d; ", chain, rulenum);
						printf("polycubectl pcn-iptables chain %s insert id=%d ", chain, rulenum);
					} else {
						printf("polycubectl pcn-iptables chain %s insert id=%d ", chain, rulenum);
					}
					break;
				...
				// replace or insert without rule specification is not permitted
				return;
			}
		} else if (delete_match == 0) {
			if (rule == NULL) {
			...
			if (rule == NULL) {
				...
				case TABLE_RAW: 
					if (append_match == 0) {
						printf("polycubectl pcn-iptables chain OUTPUT %s ", action);
					} else {
						printf("polycubectl pcn-iptables chain OUTPUT rule del ");
					}
					break;
				case TABLE_MANGLE: 
					printf("polycubectl pcn-iptables mangle %s %s ", chain, action);
	...
	// DEBUG; example: 
	// `pcn-iptables -t raw -I INPUT -s 10.0.0.1 -j` DROP gives
	// `polycubectl pcn-iptables chain INPUT insert id=0 src=10.0.0.1/32 action=DROP`
	// fprintf(stderr, debug_buffer);
	// fprintf(stderr, "\n");
	// exit(1);
		...
		struct xtc_handle *handle)
{
	...
	save_n_print_pcn_rule(chain, "insert", rule, rulenum, handle);
		 ...
		 struct xtc_handle *handle)
{
	...
	save_n_print_pcn_rule(chain, "replace", rule, rulenum, handle);
		...
		struct xtc_handle *handle)
{
	...
	save_n_print_pcn_rule(chain, "append", rule, -1, handle);
			...
			bool dry_run)
{
	...
	save_n_print_pcn_rule(chain, "delete", rule, -1, handle);
		    ...
		    struct xtc_handle *handle)
{
	...
	save_n_print_pcn_rule(chain, "delete", rule, rulenum, handle);
...
TC_FLUSH_ENTRIES(const IPT_CHAINLABEL chain, struct xtc_handle *handle)
{
	...
	save_n_print_pcn_rule(chain, "flush", ipt , -1, handle);
