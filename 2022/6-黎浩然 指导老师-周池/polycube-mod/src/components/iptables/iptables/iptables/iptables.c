		...
		struct xtc_handle **handle, bool restore)
{
	...
	if (e != NULL) {
		// set rule entry from which table it comes
		if (strcmp(*table, "raw") == 0) {
			e->comefrom = TABLE_RAW;
		} else if (strcmp(*table, "mangle") == 0) {
			e->comefrom = TABLE_MANGLE;
		} else if (strcmp(*table, "nat") == 0) {
			e->comefrom = TABLE_NAT;
		} else if (strcmp(*table, "filter") == 0) {
			e->comefrom = TABLE_FILTER;
		} else { // won't happen because of the previous check.
			e->comefrom = TABLE_UNKNOWN;
			xtables_error(PARAMETER_PROBLEM, "no table named %s in iptables", *table);
		}
	}
