...
class SegmentationMetric(object):
    ...
    def genConfusionMatrix(self, imgPredict, imgLabel):  
        ...
        mask = (imgLabel >= 0) & (imgLabel < self.numClass)   # mask=[0, 1, 1, 0]
...
if __name__ == '__main__':
    ...
    imgPredict = np.array(cv2.cvtColor(
        imgPredict, cv2.COLOR_BGR2GRAY) / 255., dtype=np.uint8)
    imgLabel = np.array(cv2.cvtColor(
        imgLabel, cv2.COLOR_BGR2GRAY) / 255., dtype=np.uint8)
