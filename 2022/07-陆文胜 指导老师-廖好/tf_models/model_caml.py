...
class Model:
    ...
    def __init__(self, vocab_size, args, user_vector, item_vector, char_vocab=0, pos_vocab=0,
        ...
        self.att1, self.att2 = [], []
        self.build_graph()
        # self.keras_model = keras.Model(inputs=[
        #     self.q1_inputs,
        #     self.q2_inputs,
        #     self.q1_len,
        #     self.q2_len,
    ...
    def _joint_representation(self, q1_embed, q2_embed, q1_len, q2_len, q1_max,
        ...
        if self.args.implicit == 1:
            q1_output = tf.concat(
                [q1_output, self.user_batch, self.user_emd_batch], 1)
                # [q1_output, self.user_batch], 1)
            q2_output = tf.concat(
                [q2_output, self.item_batch, self.item_emd_batch], 1)
            ...
            with tf.name_scope("train"):
                with tf.name_scope("cost_function"):
                    if("SOFT" in self.args.rnn_type):
                        ...
                        yt = tf.expand_dims(self.sig_labels, axis=1)
                        # yt = self.sig_labels
                        ...
                        x1 = tf.multiply(yt, tf.log(yp+1e-24))

                        x2 = tf.subtract(one_tensor, yt)
                        x3 = tf.log(tf.subtract(one_tensor, yp)+1e-24)
                    ...
                    with tf.name_scope('generation_results'):               # infer
                        self.gen_results = self._beam_search_infer(
                            q1_output, q2_output, r_input)

                    with tf.name_scope('generation_loss'):                  # train
