...
parser.add_argument('--sequence-length', type=int, metavar='N', help='sequence length for training', default=3) #序列长度，默认为3
parser.add_argument('--rotation-mode', type=str, choices=['euler', 'quat'], default='euler', # 表示旋转的方式，欧拉角或者是四元数
                    help='rotation mode for PoseExpnet : euler (yaw,pitch,roll) or quaternion (last 3 coefficients)')
parser.add_argument('--padding-mode', type=str, choices=['zeros', 'border'], default='zeros', # 填充方式，双线性插值用到？这里说是image warping用到，双线性插值应该就是warp的其中一步
                    help='padding mode for image warping : this is important for photometric differenciation when going outside target image.'
                         ' zeros will null gradients outside target image.'
                         ' border will only null gradients of the coordinate outside (x or y)')
parser.add_argument('--with-gt', action='store_true', help='use depth ground truth for validation. ' # 验证集中使用深度的ground truth
                    'You need to store it in npy 2D arrays see data/kitti_raw_loader.py for an example')
parser.add_argument('--with-pose', action='store_true', help='use pose ground truth for validation. ' # 验证集中使用位姿的ground truth
...
parser.add_argument('-j', '--workers', default=4, type=int, metavar='N', # 使用多少个线程加载数据
                    help='number of data loading workers')
parser.add_argument('--epochs', default=200, type=int, metavar='N', # 训练轮次，默认200轮
                    help='number of total epochs to run')
parser.add_argument('--epoch-size', default=0, type=int, metavar='N', # 每一轮的大小，默认为0，也就是dataset的大小
                    help='manual epoch size (will match dataset size if not set)')
parser.add_argument('-b', '--batch-size', default=4, type=int, # batch_size，默认为4，小批量
                    metavar='N', help='mini-batch size')
parser.add_argument('--lr', '--learning-rate', default=2e-4, type=float, # 学习率
                    metavar='LR', help='initial learning rate')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M', # 动量momentum
                    help='momentum for sgd, alpha parameter for adam')
parser.add_argument('--beta', default=0.999, type=float, metavar='M', # beta参数
                    help='beta parameters for adam')
parser.add_argument('--weight-decay', '--wd', default=0, type=float, # 权重衰减，默认为0，不衰减
                    metavar='W', help='weight decay')
parser.add_argument('--print-freq', default=10, type=int, # 打印频率？
                    metavar='N', help='print frequency')
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true', # 在验证集上进行验证
                    help='evaluate model on validation set')
parser.add_argument('--pretrained-disp', dest='pretrained_disp', default=None, metavar='PATH', # dispnet预训练模型的路径
                    help='path to pre-trained dispnet model')
parser.add_argument('--pretrained-exppose', dest='pretrained_exp_pose', default=None, metavar='PATH', # exp pose net预训练模型
                    help='path to pre-trained Exp Pose net model')
parser.add_argument('--seed', default=0, type=int, help='seed for random functions, and network initialization') # 随机种子，模型的可复现性相关
parser.add_argument('--log-summary', default='progress_log_summary.csv', metavar='PATH', # 存放日志文件，每一个epoch和验证
                    help='csv where to save per-epoch train and valid stats')
parser.add_argument('--log-full', default='progress_log_full.csv', metavar='PATH', # 日志文件，每次梯度下降时记录
                    help='csv where to save per-gradient descent train stats')
parser.add_argument('-p', '--photo-loss-weight', type=float, help='weight for photometric loss', metavar='W', default=1) # photometric loss的权重
parser.add_argument('-m', '--mask-loss-weight', type=float, help='weight for explainabilty mask loss', metavar='W', default=0) # mask loss的权重
parser.add_argument('-s', '--smooth-loss-weight', type=float, help='weight for disparity smoothness loss', metavar='W', default=0.1) # smooth loss的权重
parser.add_argument('--log-output', action='store_true', help='will log dispnet outputs and warped imgs at validation step') # 记录outputs
parser.add_argument('-f', '--training-output-freq', type=int, # 输出dispnet的输出和warped的imgs
...
def main():
    global best_error, n_iter, device # 全局变量记录最优error、训练轮数、设备
    args = parser.parse_args()
    if args.dataset_format == 'stacked': # stacked是什么数据集形式？
        from datasets.stacked_sequence_folders import SequenceFolder
    elif args.dataset_format == 'sequential':
        from datasets.sequence_folders import SequenceFolder
    save_path = save_path_formatter(args, parser) # 存储checkpoints的格式，返回存储路径（Path对象）
    ...
    ) # 通过SequenceFolder获取数据时会返回：tgt_img, ref_imgs, intrinsics, np.linalg.inv(intrinsics)

    # if no Groundtruth is avalaible, Validation set is the same type as training set to measure photometric loss from warping
    if args.with_gt:
        if args.with_pose:
            from datasets.validation_folders import ValidationSetWithPose
            # 验证集有位姿和深度的ground truth时
    ...
    # train_set就是SequenceFolder对象，SequenceFolder继承自torch.utils.data.Dataset
    # 传入dataset得到一个dataloader，在训练的时候就可以读取数据了
    ...
    # disp_net:disparity视差网络,sfm-learner借鉴了dispnet的结构（其实对应的就是depth_net吧）
    disp_net = models.DispNetS().to(device)
    output_exp = args.mask_loss_weight > 0 # explainability网络？如果mask_loss_weigth > 0就会用到这个网络是吗
    if not output_exp:
        print("=> no mask loss, PoseExpnet will only output pose") # 如果没用可解释性网络，PoseExpnet就只输出位姿
    pose_exp_net = models.PoseExpNet(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0).to(device)

    if args.pretrained_exp_pose: # 使用预训练的exp和pose网络
        print("=> using pre-trained weights for explainabilty and pose net")
        weights = torch.load(args.pretrained_exp_pose)
        pose_exp_net.load_state_dict(weights['state_dict'], strict=False) # 加载权重参数
    else:
        pose_exp_net.init_weights() # 否则初始化权重参数

    if args.pretrained_disp: # 使用预训练的dispnet
        print("=> using pre-trained weights for Dispnet")
        weights = torch.load(args.pretrained_disp)
        disp_net.load_state_dict(weights['state_dict'])
    else:
        disp_net.init_weights() # 否则初始化权重参数

    cudnn.benchmark = True # cudnn加速，置这个 flag 可以让内置的 cuDNN 的 auto-tuner 自动寻找最适合当前配置的高效算法，在输入数据变化不大时采用可以提高效率，当数据变化大时，每次都要搜索的话会带来额外开销
    # 是不是通过DataParallel实现双卡模式呢？网络联合优化，不一定，需要看看后面，应该不是
    disp_net = torch.nn.DataParallel(disp_net) # DataParallel可以实现多卡的数据并行训练
    ...
    # 优化器，采用Adam算法，需要设置momentum、beta和权重衰减
    ...
    with open(args.save_path/args.log_full, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['train_loss', 'photo_loss', 'explainability_loss', 'smooth_loss']) # 四种损失

    # TermLogger,自己编写的一个Logger，应该是负责输出控制台信息
    logger = TermLogger(n_epochs=args.epochs, train_size=min(len(train_loader), args.epoch_size), valid_size=len(val_loader))
    logger.epoch_bar.start()

    # 如果采用预训练过的模型或者需要采取验证的话
    # 根据是否有ground_truth采取不同的验证方法
    ...
    for epoch in range(args.epochs):
        ...
        # evaluate on validation set，跟上面一段是一样的
        # ？那假如不采用验证呢？少了一个判断，和上面不一样
        logger.reset_valid_bar()
        if args.with_gt and args.with_pose:
        ...
        decisive_error = errors[1] #取决于选择哪个最相关的误差来衡量模型的表现，并将这个误差赋值给decisive_error
        if best_error < 0:
            best_error = decisive_error

        # remember lowest error and save checkpoint
        is_best = decisive_error < best_error # 该轮误差是否小于最优误差（注意如果选择了一些越大越好的衡量指标，比如acc1、2、3则需要修改）
...
def train(args, train_loader, disp_net, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
    # 训练函数，代表一个epoch
    global n_iter, device # 全局变量
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter(precision=4)
    # w1，w2，w3是三个损失在最终损失函数里面的权重？w1默认就是1
    ...
    # 可见通过DataLoader这个接口，返回一个遍历器，加上enumerate与i对应上
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        log_losses = i > 0 and n_iter % args.print_freq == 0 # 每十轮输出一次？
        ...
        # compute output
        disparities = disp_net(tgt_img) # 传入目标图片（当前帧）计算深度网络的输出
        depth = [1/disp for disp in disparities] # depth = 1/disp?
        explainability_mask, pose = pose_exp_net(tgt_img, ref_imgs) # 可解释性掩模和位姿则通过pose_exp_net得到

        # 通过光度重建误差函数获取loss_1，warped，diff是啥？
        loss_1, warped, diff = photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics,
                                                               depth, explainability_mask, pose,
                                                               args.rotation_mode, args.padding_mode)
        if w2 > 0:
            loss_2 = explainability_loss(explainability_mask) # 如果w2大于0，说明可解释性损失也计算在内
        else:
            loss_2 = 0
        loss_3 = smooth_loss(depth) # 平滑损失

        loss = w1*loss_1 + w2*loss_2 + w3*loss_3 # 最终的损失函数
        ...
        # compute gradient and do Adam step，梯度反向传播
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        # measure elapsed time，计算运行时间
