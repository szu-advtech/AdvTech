...
class RandomHorizontalFlip(object):
    ...
    # 随机水平翻转给定的numpy array，以0.5为概率，因为还是numpy array所以是HWC
    # 相当于数据增广
    ...
    def __call__(self, images, intrinsics):
        ...
        if random.random() < 0.5:
            ...
            output_images = [np.copy(np.fliplr(im)) for im in images] # 翻转之后的图片
            w = output_images[0].shape[1] # 翻转之后的图片宽度
            output_intrinsics[0,2] = w - output_intrinsics[0,2] # 内参的[0,2]变为宽度减去原有内参[0,2]？
        else:
