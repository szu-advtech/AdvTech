...
def CNMF(MSI, HSI, mask=0, verbose='off', MEMs=0):
    ...
    HSI, MSI, W_hyper, H_hyper, W_multi, H_multi, RMSE_h, RMSE_m = CNMF_init(rows1, cols1, w, M, HSI, MSI, sum2one, I1,
                                                                             th_h, th_m, R, init_mode, mask, verbose,
                                                                             MEMs)
    ...
    # CNMF Iteration
    for i in range(I2):
        W_hyper, H_hyper, W_multi1, H_multi1, W_multi2, H_multi2, RMSE_h, RMSE_m = CNMF_ite(rows1, cols1, w, M + M_m,
                                                                                            HSI, MSI, W_hyper, H_hyper,
                                                                                            W_multi, H_multi, I1, th_h,
                                                                                            th_m, I2, i, R, mask,
                                                                                            verbose)
        ...
        if (cost[0, i] - cost[0, i + 1]) / cost[0, i] > th2 and (cost[1, i] - cost[1, i + 1]) / cost[
            1, i] > th2 and i < I2 - 1:
...
def CNMF_init(xdata, ydata, w, M, hyper, multi, delta, I_in, delta_h, delta_m, srf, init_mode=0, mask=0, verbose='off',
              MEMs=0):
    '''
    COUPLED NONNEGATIVE MATRIX FACTORIZATION (CNMF)
...
def CNMF_ite(xdata, ydata, w, M, hyper, multi, W_hyper, H_hyper, W_multi, H_multi, I_in, delta_h, delta_m, I_out, i_out,
             srf, mask=0, verbose='off'):
    '''
    COUPLED NONNEGATIVE MATRIX FACTORIZATION (CNMF)
            ...
            for x in range(hx):
                for y in range(hy):
                    if x == 0 or x == hx - 1 or y == 0 or y == hy - 1:
                        HSI[x, y, :] = (np.double(data[x * w:(x + 1) * w, y * w:(y + 1) * w, :]) * np.tile(H1, (
                        1, 1, band))).sum(axis=0).sum(axis=0).reshape(1, 1, band)
                    else:
                        HSI[x, y, :] = (np.double(
                            data[x * w - w // 2:(x + 1) * w + w // 2, y * w - w // 2:(y + 1) * w + w // 2,
                            :]) * np.tile(H2, (1, 1, band))).sum(axis=0).sum(axis=0).reshape(1, 1, band)
        else:
            H1 = gaussian_filter2d((w, w), sig).reshape(w, w, 1)
            ...
            for x in range(hx):
                for y in range(hy):
                    if x == 0 or x == hx - 1 or y == 0 or y == hy - 1:
                        ...
                        HSI[x, y, :] = (np.double(data[x * w - (w - 1) // 2:(x + 1) * w + (w - 1) // 2,
                                                  y * w - (w - 1) // 2:(y + 1) * w + (w - 1) // 2, :]) * np.tile(H2, (
                        1, 1, band))).sum(axis=0).sum(axis=0).reshape(1, 1, band)
    else:  # with mask
        if np.mod(w, 2) == 0:
            H1 = gaussian_filter2d((w, w), sig).reshape(w, w, 1)
                            ...
                            HSI[x, y, :] = (np.double(data[x * w - (w - 1) // 2:(x + 1) * w + (w - 1) // 2,
                                                      y * w - (w - 1) // 2:(y + 1) * w + (w - 1) // 2, :]) * np.tile(H2,
                                                                                                                     (1,
                                                                                                                      1,
                                                                                                                      band))).sum(
                                axis=0).sum(axis=0).reshape(1, 1, band)
...
def zoom_nn(data, w):
    ...
    rows = data.shape[0]
    cols = data.shape[1]
    out = np.tile(np.tile(data.reshape(rows, cols, 1), (1, 1, w)).reshape(rows, cols * w, 1), (1, 1, w)).transpose(1, 0,
                                                                                                                   2).reshape(
        cols * w, rows * w).transpose()
...
def zoom_bi(data, w):
    ...
    out = w4 * data[np.floor(r).astype(int), np.floor(c).astype(int)] + w3 * data[
        np.floor(r).astype(int), np.floor(c).astype(int) + 1] + w2 * data[
              np.floor(r).astype(int) + 1, np.floor(c).astype(int)] + w1 * data[
              np.floor(r).astype(int) + 1, np.floor(c).astype(int) + 1]
...
def estR(HS, MS, mask=0):
    ...
    HS = np.hstack((HS.reshape(rows2 * cols2, bands2), mask.reshape(rows2 * cols2, 1))).reshape(rows2, cols2,
                                                                                                bands2 + 1)
    bands2 = HS.shape[2]
...
def SAM(ref, tar, mask=0):
    ...
    sam_mean = np.real(
        np.arccos(prod_scal[(prod_norm != 0) * (mask == 1)] / prod_norm[(prod_norm != 0) * (mask == 1)]).sum() / (
                    (prod_norm != 0) * (mask == 1)).sum()) * 180 / np.pi
