...
%         [ POP ] = initialize_pop(N,c,bu,bd);    %初始化种群：N为种群中个体的个数，c为维度
        [ POP ] = initialize_pop_lhs(N,c,bu,bd);
       ...
       %% 近邻传播聚类(划分子种群)
        ...
        Roverlap = 0.6; %重叠率
        Rexcl = 0.5*(1/size(slst,2))*(bu - bd)/(size(slst,2)^(1/c)); %Rexcl为子种群半径
...
%                     [Exactly,condition,citr,slst,X, f, v, Pbestx, Pbestf,b,state] = Global_search1(Data,bu,bd,condition,slst,n,maxFES,citr,X, f, v, Pbestx, Pbestf, Gbestx, Gbestf);
%                     [Exactly,condition,citr,slst,X, f, v, Pbestx, Pbestf,b,state] = Global_search2(Data,bu,bd,condition,slst,n,maxFES,citr,X, f, v, Pbestx, Pbestf, Gbestx, Gbestf);
...
%                     fbest(FES,time)=min(Exactly(:,c+1));
                    ...
                    if min(Data(:,c+1)) < minData
                        ...
                        Xmax=D1(xmax,:);
                        Xmax = Xmax(:,1:c);
                        Rexcl=norm(Xmin-Xmax)/2;    %置信域半径△k
                    end
