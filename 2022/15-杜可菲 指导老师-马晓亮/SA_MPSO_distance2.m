...
%         while FES < maxFES
            ...
            switch StepCur
              ...
              %% 记录某点到种群内其他点的距离之和
                ...
                for i = 1:size(D,1)
                    distance = sum(abs(Pl(:,1:c)-repmat(Pl(i,1:c),size(Pl,1),1)).^2,2).^(1/2); %计算欧氏距离
                    a1111=find(distance < 0.001);
                    a1111(find(a1111==i)) = [];
                    a111=[a111;a1111];
                    
                    sum_distance(i) = sum(distance(:,1));
                end
                sum_distance = sum_distance';
                
%                 Pl(a111,:) = [];
%                 distance = sum(abs(Pl(:,1:c)-repmat(meanX(:,1:c),size(Pl,1),1)).^2,2).^(1/2); %计算与当前最优解的距离
%                 [a1, b]= sortrows(distance,1);%对距离进行排序，其中a为距离，b为对应的下标

                [a1, b]= sortrows(sum_distance,1);%对距离之和进行排序，其中a为距离之和，b为对应数值在sum_distance[]中的下标
...
%                         Datal1 = Pl(b(1:nl-1),:);
%                         length_b = size(b,1);
%                         Datal2 = Pl(b(length_b),:);
%                         Datal = [Datal1;Datal2];
                        
                      %% 增加一个距离当前样本点集距离最大的点
                        Datal1 = Pl(b(1:nl-1),:);
                        length_sumdis = size(b,1);
                        Datal2 = Pl(b(length_sumdis),:);
                        Datal = [Datal1;Datal2];
...
%                         FES = FES+1;
                        ...
                        if gbest(:,c+1) < min(Data(:,c+1))
                            StepNext =  2;
                            ro=(meanX(:,c+1)-gbest(:,c+1))/(gbestxf - gbestf-10^(-20)); %ro表示建立的局部RBF模型在第k次预测最优x处的准确性
