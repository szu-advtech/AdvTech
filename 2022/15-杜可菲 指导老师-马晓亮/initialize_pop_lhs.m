function [ POP ] = initialize_pop_lhs(n,c,bu,bd)
...
% figure(1)
% plot(POP(:,1),POP(:,2),'bo')
% print(gcf, '-djpeg', 'fig1.jpg') 

POP = lhsdesign(n,c,'smooth','off','criterion','maximin','iterations',50).*(ones(n,1).*(bu-bd))+ones(n,1).*bd;
% figure(2)
% plot(POP(:,1),POP(:,2),'bo')
% print(gcf, '-djpeg', 'fig2.jpg') 
end