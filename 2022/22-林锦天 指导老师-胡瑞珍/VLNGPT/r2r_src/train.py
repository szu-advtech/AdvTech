...
import argparse
import os
os.chdir('/Recurrent-VLN-BERT-WZH')

# args.angleFeatSize = 128
...
def train(train_env, tok, n_iters, log_every=2000, val_envs={}, aug_env=None):
    ...
    ##################3
    # 使用辅助数据
    args.aug  = "data/prevalent/prevalent_aug.json"
    ##################
    start_iter = 0
    ...
    ############################################
    listner.Projector.load_state_dict((torch.load('pjM204408.pth')))
    listner.gpt2.load_state_dict((torch.load('gptM204408.pth')))
    ...
    for idx in range(start_iter, start_iter + n_iters, log_every):
                    ...
                    if env_name in best_val:
                        if val > best_val[env_name]['spl']:
                            ...
                            best_val[env_name]['update'] = True
                                ...
                                score_summary['success_rate'] > best_val[env_name]['sr']):
                            best_val[env_name]['spl'] = val
                            best_val[env_name]['update'] = True
        ...
        ###########################################
        if count % 100 == 0:
            torch.save(listner.Projector.state_dict(),
                       'pjM' + str(iter) + '.pth')
            torch.save(listner.gpt2.state_dict(),
                       'gptM' + str(iter) + '.pth')
            print(listner.Projector)

            print(listner.Projector.state_dict())
            print("save!")
        print(count)
