...
# change:yjuny:噪声等级 让有些样本有noisy_rate的概率候选标签集合里面没有真值
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # change:dist:样本长度的tensor张量  is_rel:样本长度的布尔型tensor张量(判断哪个最可靠)
    sel_stats = {
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # change:yjuny:该更新prototype了 更新sel_stats选择(dist, is_rel)可以用来更新的样本集合
...
def train(args, train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, tb_logger, sel_stats):
    ...
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
        ...
        # print:
        # print('index:', i)
        # print('cls_out:', cls_out, cls_out.shape)
        # print('features_cont:', features_cont, features_cont.shape)

        # print:
        # a = copy.deepcopy(pseudo_labels)
        # print('pseudo_labels:', a, a.shape, type(a))
        # b = copy.deepcopy(target)
        # print('target:', b, b.shape, type(b))

        # 保存最后的epoch中的pseudo_labels与labels
        t1 = time.time()
        if epoch+1 == args.epochs:
            for i in range(args.batch_size):
                pseudo_labels_list.append(features_cont.cpu().detach().numpy().tolist()[i])
                target_list.append(target.cpu().detach().numpy().tolist()[i])
        ...
        if start_upd_prot:
            ...
            # yjuny:get positive set by contrasting predicted labels
            ...
            # change:yjuny:cancat clean label and guessed noisy labels
...
# change:yjuny:选择最可靠干净的最近的原型向量
def reliable_set_selection(args, epoch, sel_stats):
    dist = sel_stats['dist']
    n = dist.shape[0]
    is_rel = torch.zeros(n).bool().cuda()
    # change:yjuny:按样本与原型向量的距离dist升序返回索引
    sorted_idx = torch.argsort(dist)
    # change:yjuny:根据pure_ratio选择多少需要的样本数量 即(Top-chosen_num)
    chosen_num = int(n * args.pure_ratio)
    # change:yjuny:is_rel表示那些样本需要选
    is_rel[sorted_idx[:chosen_num]] = True
    # change:yjuny:最后更新sel_stats['is_rel']
    sel_stats['is_rel'] = is_rel
    # yjuny:select near-prototype samples as reliable and clean
