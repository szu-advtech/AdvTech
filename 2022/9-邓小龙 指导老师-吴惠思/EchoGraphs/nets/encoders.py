...
class image_encoder(nn.Module):
    ...
    def __init__(self, backbone):
        ...
        if backbone == 'alexnet':
            ...
            model_list.append(nn.Flatten(start_dim=1))
            self.base_net = nn.Sequential(*model_list)
            self.img_feature_size = 2048
        elif backbone == 'resnet48' or backbone == 48:
            self.base_net =torch.hub.load(hub_name, 'resnet50', pretrained=True)
...
class video_encoder(nn.Module):
    ...
    def __init__(self, backbone):
        ...
        if backbone == 'r3d_18':
            ...
            self.base_net = torchvision_models.video.r2plus1d_18(pretrained=True)
            self.base_net = nn.Sequential(*list(self.base_net.children())[:-1])
            self.img_feature_size = 512
        else:
            raise NotImplementedError(
...
if __name__ == '__main__':
    ...
    batch_size = 4
    number_frames = 64
    img = torch.rand(batch_size, 3, 112, 112).cuda()
    kpts_gt = torch.rand(batch_size, 8, 2).cuda()
    # m = video_encoder(backbone="r2plus1d_18").cuda()
    # m = video_encoder(backbone="r3d_18").cuda()
    # m = video_encoder(backbone="m3d_18").cuda()
    m = image_encoder(backbone='resnet50').cuda()
