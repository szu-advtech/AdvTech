...
from smoothgradcampp.sgcpp import CAM,SmoothGradCAMpp
...
def get_last_conv_name(net):
    ...
    """
    layer_name = None
    for name, m in net.named_modules(): # 返回网络中所有模块的迭代器，同时产生模块的名称以及模块本身。
...
def prepare_input(image):
    ...
    # 归一化
    means = np.array([0.485, 0.456, 0.406])#RGB
    stds = np.array([0.229, 0.224, 0.225])#正则化
    image -= means
    image /= stds

    image = np.ascontiguousarray(np.transpose(image, (2, 0, 1)))  # channel first  #ascontiguousarray将内存不连续的空间转换成内存连续的空间
    image = image[np.newaxis, ...]  # 增加 batch维度
...
def gen_cam(image, mask):
    ...
    """
    # mask转为heatmap
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)#产生伪彩色图像
...
def main(args):
    ...
    # img = io.imread("examples/pic1.jpg")
    ...
    # sgcpp
    sgcppBase = SmoothGradCAMpp(net, layer_name)
    smask = sgcppBase(inputs)
    image_dict['smooth-cam++'], image_dict['smooth-hearmap++'] = gen_cam(img, smask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight-path', type=str, default='/Users/liuxintong/Documents/program/models/pytorch/densenet121-a639ec97.pth',
    ...
    parser.add_argument('--class-id', type=int, default=None,
                        help='class id')
    parser.add_argument('--output-dir', type=str, default='abc',
