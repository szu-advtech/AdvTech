...
void *client_thread(void *arg)
{
    ...
    /* initializes the structures, iobuffer 多线程用途 */
    init_iobuffer(&iobuf);
    ...
    // 将请求内容写入 buffer，主要是从 lcfd.fd 本地文件描述符中读取
    // 从客户端读取一行数据，以换行符为结束
    ...
    /* strstr表示在 buffer 中找到第一次出现后面字符串的位置 */
    ...
    if (query_suffixed)
    {
        ...
        {                                           // there is an _ in the url so the input number should be present
            ...
            if ((req.type == A_SNAPSHOT_WXP) || (req.type == A_STREAM_WXP))
            { // webcamxp adds offset to the camera number
                input_number--;
            }
        }
        DBG("plugin_no: %d\n", input_number);
    ...
    /*
     * parse the rest of the HTTP-request 解析剩余的http报文信息，如 User-Agent
    ...
    do
    {
        ...
        // 如果包含了密码，则将密码保存至req.credentials
    ...
    /*  根据请求类型，处理如何发送数据 now it's time to answer */
    if (query_suffixed)
    {
...
void *server_thread(void *arg)
{
    ...
    for (aip2 = aip; aip2 != NULL; aip2 = aip2->ai_next)
    {
        ...
        // TCP Server 的初始化过程，并且设置的最多 listen 10 个 client
    ...
    /* 为 accept 的每一个 client 创建单独的线程，用于处理请求和发送数据 */
