...
class Scholar(object):
    ...
    def fit(self, X, tfidf, Y, PC, TC, eta_bn_prop=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        """
        ...
        tfidf = torch.Tensor(tfidf).to(self.device)
        if Y is not None:
        ...
        # do a forward pass
        thetas, X_recon, Y_probs, losses = self._model(X, tfidf, Y, PC, TC, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci)
        loss, nl, kld = losses
        # update model
        loss.backward()
        self.optimizer.step()
    ...
    def predict(self, X, tfidf, PC, TC, eta_bn_prop=0.0):
        """
        ...
        if TC is not None:
            TC = torch.Tensor(TC).to(self.device)
        theta, _, Y_recon, _ = self._model(X, tfidf, Y, PC, TC, do_average=False, var_scale=0.0, eta_bn_prop=eta_bn_prop)
    ...
    def get_losses(self, X, tfidf, Y, PC, TC, eta_bn_prop=0.0, n_samples=0):
        """
        ...
        if batch_size == 1:
            ...
            tfidf = np.expand_dims(tfidf, axis=0)
        ...
        if TC is not None:
            TC = torch.Tensor(TC).to(self.device)
        if n_samples == 0:
            _, _, _, temp = self._model(X, tfidf, Y, PC, TC, do_average=False, var_scale=0.0, eta_bn_prop=eta_bn_prop)
            loss, NL, KLD = temp
            losses = loss.to('cpu').detach().numpy()
        else:
            _, _, _, temp = self._model(X, tfidf, Y, PC, TC, do_average=False, var_scale=1.0, eta_bn_prop=eta_bn_prop)
            loss, NL, KLD = temp
            losses = loss.to('cpu').detach().numpy()
    ...
    def compute_theta(self, X, tfidf, Y, PC, TC, eta_bn_prop=0.0):
        """
        ...
        if TC is not None:
            TC = torch.Tensor(TC).to(self.device)
        theta, _, _, _ = self._model(X, tfidf, Y, PC, TC, do_average=False, var_scale=0.0, eta_bn_prop=eta_bn_prop)
...
class torchScholar(nn.Module):
    ...
    def __init__(self, config, alpha, update_embeddings=True, init_emb=None, bg_init=None, device='GPU', classify_from_covars=False):
    ...
    def forward(self, X, tfidf, Y, PC, TC, compute_loss=True, do_average=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        """
...
class torchContrastiveScholar(nn.Module):
    ...
    def __init__(self, config, alpha, update_embeddings=True, init_emb=None, bg_init=None, device='GPU', classify_from_covars=False, topk=1):
    ...
    def forward(self, X, tfidf, Y, PC, TC, compute_loss=True, do_average=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        # dir = 'D:/Code/Python/CLNTM-master/data/20ng/processed/'
        # prefix = 'train'
        # tfidf = compute_tfidf.get_tfidf(dir, prefix)
        # tfidf = tfidf.todense()
        # print(tfidf)
        # Apt = torch.tensor(tfidf)
...
class torchTrueContrastiveScholar(nn.Module):
    ...
    def __init__(self, config, alpha, update_embeddings=True, init_emb=None, bg_init=None, device='cpu', classify_from_covars=False, topk=1):
        ...
        print('torchTrueContrastiveScholar')
        # load the configuration
        self.vocab_size = config['vocab_size']
        self.words_emb_dim = config['embedding_dim']
        self.n_topics = config['n_topics']
    ...
    def forward(self, X, tfidf, Y, PC, TC, compute_loss=True, do_average=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        # negative part 
        max_ids = torch.topk(tfidf, k=self.topk, dim=1).indices
        max_values = torch.topk(X_recon,k=self.topk,dim=1).values
        total_words = torch.sum(X,1).unsqueeze(-1)
        max_values = max_values*total_words
        tmp_x = copy.copy(X)
        ...
        # positive part
        min_ids = torch.topk(tfidf, k=self.topk,largest=False, dim=1).indices
