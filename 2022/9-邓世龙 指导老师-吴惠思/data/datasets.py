...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training',
                 ...
                 root='/data/dengshilong/gmflow/datasets/Sintel',
                 dstype='clean',
                 load_occlusion=False,
                 ):
...
class FlyingChairs(FlowDataset):
    def __init__(self, aug_params=None, split='train',
        ...
        root = '/data/dengshilong/gmflow/datasets/FlyingChairs_release/data'
        images = sorted(glob(osp.join(root, '*.ppm')))
        flows = sorted(glob(osp.join(root, '*.flo')))
        assert (len(images) // 2 == len(flows))

        # split_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'chairs_split.txt')
        split_file ='/data/dengshilong/gmflow/datasets/FlyingChairs_release/chair_split.txt'
