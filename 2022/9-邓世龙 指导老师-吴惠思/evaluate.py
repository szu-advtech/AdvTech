...
def create_sintel_submission(model,
    ...
    for dstype in ['clean', 'final']:
        test_dataset = data.MpiSintel(aug_params=None, split='test', dstype=dstype)
...
def validate_sintel(model,
    ...
    for dstype in ['clean', 'final']:
        val_dataset = data.MpiSintel(split='training', dstype=dstype, load_occlusion=evaluate_matched_unmatched)
