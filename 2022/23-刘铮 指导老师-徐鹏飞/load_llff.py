...
def normalize(x):
    return x / np.linalg.norm(x)  # 除以标准差
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    for theta in np.linspace(0., 2. * np.pi * rots, N + 1)[:-1]:  # 返回指定间隔内的等距数字
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    '''
    '''
    poses中[:3,:4]代表图片的旋转矩阵和平移矩阵，第五列是hwf
    '''
    ...
    # 如果需要重定位世界坐标，计算poses的均值，将所有pose做该均值的逆转换，即重新定义世界坐标系，原点大致在被测物中心；
    if recenter:
    ...
    i_test = np.argmin(dists)  # 保存距离平均位姿最小的位姿所对应的下标，用于Holdout交叉验证（留出法）。
