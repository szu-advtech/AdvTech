...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.att1 = nn.Sequential(
            nn.Conv2d(inplanes, int(planes/4), 1, 1, bias=False),
            nn.BatchNorm2d(int(planes/4)),
            nn.ReLU(inplace=True),
        )
        self.att2 = conv3x3(int(planes/4), int(planes/4), 1)
        self.att_bn1 = nn.BatchNorm2d(int(planes/4), momentum=BN_MOMENTUM)
        self.att3 = nn.Conv2d(int(planes/4), planes, 1, 1, bias=False)
...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 32, num_joints: int = 1):
        ...
        self.mid_layer = nn.Conv2d(base_channel, 1, kernel_size=1, stride=1)
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
