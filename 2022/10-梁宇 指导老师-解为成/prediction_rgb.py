...
# from model.miccai_msnet_ori import MSNet-main
from model.miccai_msnet import MSNet
# from model.miccai_msnet_borad import MSNet
import ttach as tta

torch.manual_seed(2018)
torch.cuda.set_device(0)

ckpt_path = './saved_model/msnet'
# exp_name = 'miccai_msnet'
exp_name = 'origin_baseline'
args = {
    'snapshot': 'model-ori50',
    'crf_refine': False,
    'save_results': True
}
...
def main():
    ...
    net.load_state_dict(torch.load(os.path.join(ckpt_path, exp_name, args['snapshot']),map_location={'cuda:0': 'cuda:0'}))
    net.eval()
    with torch.no_grad():
        ...
        for name, root in to_test.items():
            ...
            for idx, img_name in enumerate(img_list):
                ...
                img_var = Variable(img_transform(img_resize).unsqueeze(0), volatile=True).to(device)
