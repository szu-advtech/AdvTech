...
def gendata(dataset_path, out_path, benchmark, part='eval'):  # benchmark = ['xsub', 'xview'] part = ['train', 'val']
    dataset = NTUMotionProcessor(
        '{}/{}_data.npy'.format(os.path.join(dataset_path, benchmark), part),  # "/home/dell/Action_Recognition/CrosSCLR/data/NTU-RGB-D/xview/"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default='data/NTU-RGB-D_120')  # 已经把数据处理好了
    parser.add_argument('--out_folder', default='data/NTU120_frame50')  # 指定输出路径
    ...
    for b in benchmark:
        for p in part:
            ...
            gendata(arg.dataset_path, out_path, benchmark=b, part=p)  # 调用gendata函数，最终生成position.npy、motion.npy和label.npy