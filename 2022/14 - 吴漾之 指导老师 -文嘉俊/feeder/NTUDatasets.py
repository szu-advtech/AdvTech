...
class NTUMotionProcessor(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.y_rotation = y_rotation
        self.t_length = t_length if t_length < self.max_length else self.max_length
        neighbor_1base = [(21, 2), (21, 3), (21, 5), (21, 9), (3, 4),  # 标注邻接关节点（间隔为1）
                          (5, 6), (6, 7), (7, 8), (8, 23), (23, 22),
                          (9, 10), (10, 11), (11, 12), (12, 25), (25, 24),
                          (2, 1), (1, 13), (1, 17), (13, 14), (14, 15),
                          (15, 16), (17, 18), (18, 19), (19, 20)]
        self.neighbor_1base = [(i - 1, j - 1) for (i, j) in neighbor_1base]  # 从0开始设置
        self.load_data(mmap)  # 调用load_data函数，将数据读取出来，获得N,C,V,T,M

    def load_data(self, mmap):  # mmap = True
        # data: N C V T M  # 视频数量 关节特征 关节的数量 帧的数量 一帧中的人数
        # load label
        with open(self.label_path, 'rb') as f:  # rb：读取二进制文件
            print(self.label_path)
            self.sample_name, self.label = pickle.load(f)
            self.label = np.array(self.label)  # 将list类型转为矩阵
            self.label = self.label - self.label.min()  # 矩阵的每一个元素减去矩阵中最小的元素
        ...
        self.N, self.C, self.T, self.V, self.M = self.data.shape  # 从数据文件中读取五个维度的参数
