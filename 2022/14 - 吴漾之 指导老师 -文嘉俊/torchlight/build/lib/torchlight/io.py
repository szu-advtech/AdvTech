...
def import_class(import_str):  # 处理键值对中值的部分，如 processor.pretrain_crossclr_3views.CrosSCLR_3views_Processor
    mod_str, _sep, class_str = import_str.rpartition('.')
    """
    rpartition() 方法搜索指定字符串的最后一次出现，并将该字符串拆分为包含三个元素的元组。
    第一个元素包含指定字符串之前的部分。第二个元素包含指定的字符串。第三个元素包含字符串之后的部分。
    """
    __import__(mod_str)  # 如 processor.pretrain_crossclr_3views
    try:
        """
        获取processor.pretrain_crossclr_3views中CrosSCLR_3views_Processor的值
        """
