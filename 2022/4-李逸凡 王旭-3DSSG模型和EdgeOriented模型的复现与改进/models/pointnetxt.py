...
from models.layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(block, str):
            block = eval(block)
        self.blocks = blocks  # [1,1,1,1,1,1]
        self.strides = strides  # [1,2,2,2,2,1]
        self.in_channels = in_channels  # 3
        self.aggr_args = aggr_args  # {'feature_type': 'dp_fj', 'reduction': 'max'}
        self.norm_args = norm_args  # {'norm': 'bn'}
        self.act_args = act_args  # {'act': 'relu'}
        self.conv_args = {'order': 'conv-norm-act'}
        self.sample_method = 'fps'
        self.expansion = expansion  # 4
        self.sa_layers = sa_layers  # 2
        self.sa_use_res = sa_use_res  # True
        self.use_res = True
        radius_scaling = 1.5
        nsample_scaling = 1

        self.radii = self._to_full_list(radius, radius_scaling)  # [[0.15], [0.15], [0.22499999999999998], [0.33749999999999997], [0.50625], [0.7593749999999999]]
        self.nsample = self._to_full_list(nsample, nsample_scaling)  # [[32], [32], [32], [32], [32], [32]]
    ...
    def forward_cls_feat(self, p0, f0=None):  # p0为点的坐标， f0为点的特征
        if f0 is None:
