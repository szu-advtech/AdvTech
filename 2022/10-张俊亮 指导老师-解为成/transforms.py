...
def color_aug(img, random_h=36, random_l=50, random_s=50):
    ...
    img = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_HLS2BGR)  ###   img[:, :, 0] / img[:, :, 1] / img[:, :, 2]：表示图像单个通道的像素。
    return img
...
class GroupRation(object):
    ...
    def __call__(self, img_group, is_mv=False):
        ...
        for img in img_group:
            ...
            (h, w) = img.shape[:2]  # 返回(高,宽,色彩通道数),此处取前两个值返回
            # 抓取旋转矩阵(应用角度的负值顺时针旋转)。参数1为旋转中心点;参数2为旋转角度,正的值表示逆时针旋转;参数3为各向同性的比例因子
            M = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1.0)
            ratoion_img=cv2.warpAffine(img, M, (w, h))  # borderValue 缺省，默认是黑色
            ration.append(ratoion_img)
        # for img in img_group:
        #     plt.imshow(img)
        #     plt.show()
        # for img2 in ration:
...
class GroupTranslate(object):
    ...
    def __call__(self, img_group, is_mv=False):
        ...
        for img in img_group:
            ...
            Translate.append(trans_img)


        return Translate


class GroupScale2(object):
    def __init__(self, is_mv=False):
        self._is_mv = is_mv

    def __call__(self, img_group, is_mv=False):
        scale_size=[1.10,1.05,.95,.90]
        scale=random.choice(scale_size)
        (h,w)=img_group[0].shape[:2]

        # im_size=(int(float(h)*scale),int(float(w)*scale))
        im_size = ( int(float(w) * scale),int(float(h) * scale))

        if img_group[0].shape[2] == 3:
            ret_img_group = [cv2.resize(img, im_size,
                                        cv2.INTER_LINEAR)
                             for img in img_group]
        elif img_group[0].shape[2] == 2:
            ret_img_group = [resize_mv(img, im_size, cv2.INTER_LINEAR)
                             for img in img_group]
...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_group):   ##将类变成一个可调用的对象
