...
parser = argparse.ArgumentParser(description="Mask trainer")
...
class Trainer(object):
        ...
        for epoch_idx in range(int(self.epochs)):
            ...
            for feature, label in self.dataloader.get_data("train", batch_size = self.bs):
                ...
                if step % 100 == 0:
                    ...
                    self.logger.info(self.network.mask_embedding.mask_weight)
            ...
            self.logger.info("[Epoch {epoch:d} | Train Loss: {loss:.6f} | Val AUC: {val_auc:.6f}, Val Loss: {val_loss:.6f}]".format(epoch=epoch_idx,  loss=train_loss, val_auc=val_auc, val_loss=val_loss))
            rate = self.network.compute_remaining_weights()
            self.logger.info("Feature remain:{rate:.6f}".format(rate=rate))
        test_auc, test_loss = self.evaluate("test")
        self.logger.info("Test AUC: {test_auc:.6f}, Test Loss: {test_loss:.6f}".format(test_auc=test_auc, test_loss=test_loss))
    ...
    def train(self, epochs):
        ...
        self.logger.info("Ticket:{t}".format(t=self.network.ticket))
        self.logger.info("Final feature remain:{rate:.6f}".format(rate=rate))
        for epoch_idx in range(int(epochs)):
            train_loss = .0
            step = 0
            for feature, label in self.dataloader.get_data("train", batch_size = self.bs):
                loss = self.train_on_batch(label, feature, retrain=True)
                train_loss += loss
                step += 1
                if step % 10 == 0:
                    self.logger.info("[Epoch {epoch:d} | Step :{setp:d} | Train Loss:{loss:.6f}".
            ...
            if val_auc > cur_auc:
                ...
                early_stop = True
                test_auc, test_loss = self.evaluate("test")
                self.logger.info("Early stop at epoch {epoch:d} | Test AUC: {test_auc:.6f}, Test Loss:{test_loss:.6f}".format(epoch=epoch_idx, test_auc = test_auc, test_loss = test_loss))
                break
        
        if not early_stop:
            test_auc, test_loss = self.evaluate("test")
            self.logger.info("Final Test AUC: {test_auc:.6f}, Test Loss: {test_loss:.6f}".format(test_auc=test_auc, test_loss=test_loss))
