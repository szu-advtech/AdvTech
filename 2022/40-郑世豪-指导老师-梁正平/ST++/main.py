...
def main(args):
    ...
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, pin_memory=True, num_workers=4, drop_last=False)
    #用best_model为选取出来的reliable_ids打标签
    label(best_model, dataloader, args)
    ...
    with torch.no_grad():
        for img, mask, id in tbar:
            ...
            for i in range(len(preds) - 1):
                metric = meanIOU(num_classes=21 if args.dataset == 'pascal' else 19)
                metric.add_batch(preds[i], preds[-1])#和最后一个相比
                mIOU.append(metric.evaluate()[-1])

            reliability = sum(mIOU) / len(mIOU) #score Si
            id_to_reliability.append((id[0], reliability))

    id_to_reliability.sort(key=lambda elem: elem[1], reverse=True)#分数排序
    #选取R个分数最高的
