...
class VAE():
    ...
    def create_params(self):
        # params of q network
        self.weights_q_auxiliary, self.biases_q_auxiliary, self.weights_q, self.biases_q = [], [], [], []
        for i, (d_in, d_out) in enumerate(zip(self.q_dims[:-1], self.q_dims[1:])):  
            if i == len(self.q_dims[:-1]) - 1:
                # we need two sets of parameters for mean and variance, respectively
                d_out *= 2 
            weight_key_auxiliary = "weight_q_auxiliary_{}to{}".format(i, i + 1)
            bias_key_auxiliary = "bias_q_auxiliary_{}".format(i + 1)
            weight_key = "weight_q_{}to{}".format(i, i+1)
            bias_key = "bias_q_{}".format(i+1)
            self.weights_q_auxiliary.append(tf.get_variable(name=weight_key_auxiliary, shape=[
                self.user_num, d_out / 2], initializer=tf.contrib.layers.xavier_initializer(
                seed=self.random_seed)))
            self.biases_q_auxiliary.append(tf.get_variable(name=bias_key_auxiliary, shape=[
                d_out / 2], initializer=tf.truncated_normal_initializer(stddev=0.001, seed=self.random_seed)))
            # W
        ...
        for i, (d_in, d_out) in enumerate(zip(self.p_dims[:-1], self.p_dims[1:])):
            ...
            self.weights_p.append(tf.get_variable(name=weight_key, shape=[
                                  d_in + self.q_dims[-1], d_out], initializer=tf.contrib.layers.xavier_initializer(seed=self.random_seed)))
    ...
    def q_graph(self):
        ...
        h = tf.nn.l2_normalize(self.input_ph_target, 1)
    ...
    def q_graph_auxiliary(self):
        mu_q_auxiliary = None

        h = tf.nn.l2_normalize(self.input_neighbor, 1)
        h = tf.nn.dropout(h, self.keep_prob_ph)

        for i, (w, b) in enumerate(zip(self.weights_q_auxiliary, self.biases_q_auxiliary)):
            h = tf.matmul(h, w) + b
            if i != len(self.weights_q_auxiliary) - 1:
                h = tf.nn.tanh(h)
            else:
                mu_q_auxiliary = h[:, :self.q_dims[-1]]  
        return mu_q_auxiliary

    def inference(self):
        # q-network-auxiliary
        mu_q_auxiliary = self.q_graph_auxiliary()
        # q-network  
        ...
        # p-network 
        self.logits_target = self.p_graph(tf.concat((sampled_z, mu_q_auxiliary), 1)) 
        self.KL = KL
    ...
    def create_loss(self):
        ...
        reg_var_auxiliary = apply_regularization(reg, self.weights_q_auxiliary)

        # tensorflow l2 regularization multiply 0.5 to the l2 norm
        # multiply 2 so that it is back in the same scale
        neg_ELBO_auxiliary = 2 * reg_var_auxiliary
        neg_ELBO = self.neg_ll + self.anneal_ph * self.KL + 2 * reg_var
        self.loss = neg_ELBO + neg_ELBO_auxiliary
