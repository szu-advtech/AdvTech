    ...
    def test(self, data_loader, res_fpath=None, gt_fpath=None, visualize=False):
        ...
        for batch_idx, (data, map_gt, imgs_gt, frame) in enumerate(data_loader):
            ...
            if visualize and batch_idx % 100 == 0:
                ...
                plt.savefig(os.path.join(self.logdir_imgpath, f'map_frame{str(frame.item())}.jpg'))
                plt.close(fig)

                # visualizing the heatmap for per-view estimation
                for view in range(7):
                    heatmap0_head = imgs_res[view][0, 0].detach().cpu().numpy().squeeze()
                    heatmap0_foot = imgs_res[view][0, 1].detach().cpu().numpy().squeeze()
                    ...
                    head_cam_result.save(
                        os.path.join(self.logdir_imgpath, f'view{view}_frame_head{str(frame.item())}_res.jpg'))
                    foot_cam_result = add_heatmap_to_image(heatmap0_foot, img0)
                    foot_cam_result.save(
                        os.path.join(self.logdir_imgpath, f'view{view}_frame_foot{str(frame.item())}_res.jpg'))
