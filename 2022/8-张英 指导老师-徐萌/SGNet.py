...
class SPCA_Block(nn.Module):
    def __init__(self, input_channel=75//2):
        ...
        # channel attention
        self.conv_ca_atten_1 = nn.Conv2d(in_channels=75//2, out_channels=(75//2 // self.reduction-1), kernel_size=(1, 1))
    ...
    def forward(self, Input_features):
        ...
        max_pool_feature = F.max_pool2d(conv_feature_sa, (conv_feature_sa.shape[2], conv_feature_sa.shape[3]))    # 全局最大池化     # 37
        ...
        average_pool_feature = F.avg_pool2d(conv_feature_sa, (conv_feature_sa.shape[2], conv_feature_sa.shape[3]))  # 全局平均池化  # 37
...
class SGNet(nn.Module):
    ...
    # 实现全局残差
    def __init__(self, input_image_channel=150):
        ...
        self.conv4 = nn.Conv2d(in_channels=150,out_channels=150,kernel_size=1)
        # self.conv5 = nn.Conv2d(in_channels=75//2,out_channels=150,kernel_size=3,stride=1,padding=1,bias=False)
        self.conv6 = nn.Conv2d(in_channels=75//2,out_channels=150,kernel_size=1)
        self.conv_dw_1=nn.Conv2d(in_channels=75//2,out_channels=75//2,kernel_size=3,padding=1,stride=1,groups=75//2)
        self.conv_dw_2=nn.Conv2d(in_channels=75//2,out_channels=150,kernel_size=1)
...
if __name__ == '__main__':
    ...
    # out = mcplb(x)
    # print(out.shape)
    summary(mcplb.cuda(), (150,256,256))