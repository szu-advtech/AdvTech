...
class DOTADataset(CustomDataset):
    ...
    def __init__(self,*arg,balance_category=False,version='1',**kwargs):
        ...
        print('开始balance:{}'.format(len(self.img_infos)))
        if balance_category:
            self.img_infos = self._balance_categories()
            self.total_len = len(self.img_infos)
        print('结束balance:{}'.format(len(self.img_infos)))
    def _balance_categories(self):
        ...
        new_idx = []
        # print(cate_dict)   # keys:123456789\10
        # for i in cate_dict.keys():
        #     print(len(cate_dict[i]),self.CLASSES[i-1])
        # 6943 bridge
        # 3462 tennis-court
        # 5928 baseball-diamond
        # 744 storage-tank
        # 9790 plane
        # 806 ground-track-field
        # 772 ship
        # 834 small-vehicle
        # 382 basketball-court
        # 682 large-vehicle
        ...
        balance_dict={
            "bridge":(1,600),
            "tennis-court":(2,400),
            "baseball-diamond":(1,500),
            "storage-tank":(10,0),
            "plane":(1,0),
            "ground-track-field":(10,0),
            "ship":(10,0),
            "small-vehicle":(10,0),
            "basketball-court":(20,60),
            "large-vehicle":(10,100)
        }
        # 原本的
        # balance_dict={
        #     "storage-tank":(1,526),
