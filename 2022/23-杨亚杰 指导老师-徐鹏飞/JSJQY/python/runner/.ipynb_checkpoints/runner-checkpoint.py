...
def SWA_optim(path_list):
    leng = len(path_list)
    res_param_sum = {}
    for file_path in path_list:
        pkl_file = open(file_path, 'rb')
        model = pickle.load(pkl_file)['model']
        for key in model.keys():
            if key not in res_param_sum.keys():
                res_param_sum[key]=model[key]/leng
            else:
                res_param_sum[key] += (model[key]/leng)
    return res_param_sum
...
class Runner:
    ...
    def run(self):
        ...
        if self.swa_begin!=-1:
            print('执行了')
            swa_points = []
            for i in range(self.swa_begin):
                work_path=os.path.join(self.work_dir,'checkpoints')
                swa_points.append(os.path.join(work_path,'ckpt_{}.pkl'.format(self.max_epoch-i)))
            myModel = SWA_optim(swa_points)
            self.model.load_parameters(myModel)
        self.test()
    ...
    def train(self):
        ...
        for batch_idx,(images,targets) in enumerate(self.train_dataset):
            ...
            if self.finish:
                ...
                # # 这里要计算和加载SWA模型参数
                # if self.swa_begin != -1:
                #     pointPath = os.path.join(self.work_dir,'checkpoints')
                #     list_points=os.listdir(pointPath)
                #     SWA_points=list_points[-self.swa_begin:]
                #     print(SWA_points)
                #     # self.model.load_parameters(SWA_optim(path=))
                break
        self.epoch +=1
