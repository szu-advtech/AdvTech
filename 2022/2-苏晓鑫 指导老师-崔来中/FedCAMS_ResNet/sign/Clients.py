...
class Client():
    ...
    def get_model_gradient(self, idx):
        ...
        # gra = torch.norm(self.accu_gra[idx], p=1) * torch.sign(self.accu_gra[idx]) / self.accu_gra[idx].shape[0]
        gra = torch.norm(parameters - last_parameters, p=1) * torch.sign(parameters - last_parameters) / \
              self.accu_gra[idx].shape[0]
        # self.accu_gra[idx] = gra - self.accu_gra[idx]
        return gra, num_batches_tracked
