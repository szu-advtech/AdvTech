...
class Client():
    def __init__(self, client_num):
        ...
        self.model = torch.load(
            "../init_conv_model.pkl")  # m.ConvMixer(256, 8, patch_size=2, kernel_size=5, n_classes=10)
        self.last_model = torch.load(
            "../init_conv_model.pkl")  # m.ConvMixer(256, 8, patch_size=2, kernel_size=5, n_classes=10)
        print(self.model)
        ...
        #     start_index = int(len(self.train_data) / self.client_num) * i
        #     print("the start index is ", start_index)
        #     data = []
        #     for j in range(data_len):
        #         idx = (start_index + j) % len(self.train_data)
        #         # data.append(train_data[index_list[idx]])
        #         data.append(self.train_data[idx])
    ...
    def get_model_gradient(self, idx):
        ...
        for p in self.last_model.parameters():
            ...
            #     print("Old Client ", idx, " ", p, " ======== ", self.last_model.state_dict()[p])
        ...
        #         print("the unGrad module name is {}".format(k))
        #         continue
        #     length = v.numel()
        #     print("the grad module name is {}, the min value is {}, the max value is {}".format(k, torch.min(
        #         self.accu_gra[idx][i:i + length]), torch.max(self.accu_gra[idx][i:i + length])))
        #     i += length
        #
        # value, indices = torch.topk(self.accu_gra[idx].abs(), int(self.accu_gra[idx].size()[0] * self.proportion))
        # print("top {}% is {}".format(self.proportion * 100, torch.min(value)))
        #
        ...
        # result = torch.sort(torch.abs(last_parameters - parameters), descending=True).cpu().numpy().tolist()
        # strNums = [str(x_i) for x_i in result]
        # str1 = ",".join(strNums)
        # file_handle.write(str1+'\n')

        return parameters - last_parameters, num_batches_tracked
