...
if __name__ == '__main__':
    ...
    while True:
        ...
        for p in model.parameters():
            ...
            deltaSqu = model_gra[idx:idx + size] ** 2
            v[idx:idx + size] = v[idx:idx + size] - (1 - beta2) * deltaSqu * torch.sign(
                v[idx:idx + size] - deltaSqu)

            model_gra[idx:idx + size] = eta * m[idx:idx + size] / (
                        torch.sqrt(v[idx:idx + size]) + epsilon[idx:idx + size])
