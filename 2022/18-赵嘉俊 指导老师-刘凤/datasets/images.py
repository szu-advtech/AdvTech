...
class BaseImageLoader(BaseSimulator):
    ...
    def load_dataset(self, train, dataset_dir, numpy=False, limit_samplesize=None, true_param_id=0, joint_score=False, ood=False, run=0):
        ...
        # Load data (all of samples) as numpy array
        ...
        if numpy:
            ...
            x = transforms.Resize((64, 64))(torch.from_numpy(x))
            x = x.unsqueeze(1).repeat(1,3,1,1).numpy()
            if np.max(x) <= 1.0:
                x = 0.5 + 255.0 * x
            assert 0.0 <= np.min(x)
...
class CardiacLoader(BaseImageLoader):
    def __init__(self):
        super().__init__(
            resolution=64, n_bits=8, random_horizontal_flips=True,
        )
