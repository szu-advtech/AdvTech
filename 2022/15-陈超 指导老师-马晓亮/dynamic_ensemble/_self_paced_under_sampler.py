...
import sys
sys.path.append("..")
from dynamic_ensemble._base_sampler import BaseSampler
from utils._validation_param import check_pred_proba, check_type
from utils._validation import _deprecate_positional_args, check_target_type
...
class SelfPacedUnderSampler(BaseSampler):
    ...
    def __init__(
        ...
        self.bin_weights_pre = bin_weights_pre
        # Check parameters
    ...
    # 作者提出的算法
    @_deprecate_positional_args
    def _fit_resample_author(self, X, y, *,
    ...
    def _undersample_single_class_author(self, hardness_c, n_target_samples_c,
    ...
    def _undersample_single_class(self, hardness_c, n_target_samples_c,
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            index_bins = []
            for i_bin in range(k_bins):
                index_bin = np.zeros_like(hardness_c, dtype='bool')
                for pair in path:
                    if pair[1] == edges[i_bin]:
                        populations[i_bin] += 1
                        index_bin[pair[0]] = True
                index_bins.append(index_bin)

            for i_bin in range(k_bins):
                index_bin = index_bins[i_bin]
                contributions[i_bin] = hardness_c[index_bin].mean()
