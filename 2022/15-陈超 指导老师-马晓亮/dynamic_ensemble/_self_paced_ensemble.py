...
import sys
sys.path.append("..")
from dynamic_ensemble.base import BaseImbalancedEnsemble, MAX_INT
from dynamic_ensemble._self_paced_under_sampler import SelfPacedUnderSampler
from utils._validation import _deprecate_positional_args
from utils._validation_data import check_eval_datasets
from utils._validation_param import (check_target_label_and_n_target_samples,
                                     check_balancing_schedule,
...
class DynamicEnsemble(BaseImbalancedEnsemble):
    ...
    def _fit(self, X, y, 
        ...
        for i_iter in range(n_estimators):
            ...
            self._update_cached_dropout_probabilities(i_iter, n_estimators,  X, y)
            self._update_forgetting_score(i_iter, y)
    ...
    def _update_cached_prediction_probabilities(self, i_iter, X):
        ...
        if i_iter == 0:
            self.y_pred_proba_latest = np.zeros((self._n_samples, self.n_classes_), 
                                                dtype=np.float64)
            self.y_pred_proba_pre = np.zeros((self._n_samples, self.n_classes_),
                                                dtype=np.float64)
        else:
            y_pred_proba_latest = self.y_pred_proba_latest
            y_pred_proba_new = self.estimators_[-1].predict_proba(X)
            self.y_pred_proba_pre = self.y_pred_proba_latest
            self.y_pred_proba_latest = (y_pred_proba_latest * i_iter + y_pred_proba_new) / (i_iter+1)
        return


    def _update_forgetting_score(self, i_iter, y):
        if i_iter == 0:
            self.forgetting_score = np.zeros((self._n_samples, ), dtype='float64')
            self.forget_cnt = np.zeros((self._n_samples, ), dtype='int')
            self.is_learnt = np.zeros((self._n_samples, ), dtype='bool')

        elif i_iter == 1:
            y_pred_latest = np.argmax(self.y_pred_proba_latest, axis=1)
            self.is_learnt = (y_pred_latest == y)
        else:
            y_pred_pre = np.argmax(self.y_pred_proba_pre, axis=1)
            y_pred_latest = np.argmax(self.y_pred_proba_latest, axis=1)
            y_pred_pre_mask = (y_pred_pre == y)
            y_pred_latest_mask = (y_pred_latest == y)
            learn_mask = (y_pred_latest_mask > y_pred_pre_mask)
            self.is_learnt[learn_mask] = True
            mask_forget = (self.is_learnt & (y_pred_pre_mask > y_pred_latest_mask))
            self.forget_cnt += mask_forget.astype('int')
            self.forgetting_score = self.forget_cnt / i_iter



    def _update_cached_dropout_probabilities(self, i_iter, n_estimators, X, y):
        if i_iter == 0:
            self.dropout_proba = np.zeros((self._n_samples, ), dtype=np.float64)
            self.dropout_cnt = np.zeros((self._n_samples,), dtype=np.int64)

        else:
            y_pred = self.estimators_[-1].predict(X)
            mask = (y_pred != y)
            self.dropout_cnt[mask] += 1
            self.dropout_cnt[~mask] = 0
            # self.dropout_proba = self.dropout_cnt * (i_iter / n_estimators)
            self.dropout_proba = self.dropout_cnt / (i_iter + 1)
        return
