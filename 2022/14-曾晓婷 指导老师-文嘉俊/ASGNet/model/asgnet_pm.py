...
class Model(nn.Module):
        ...
        if self.pyramid:
            ...
            for bin in self.pyramid_bins:
                self.corr_conv.append(nn.Sequential(
                    nn.Conv2d(reduce_dim * 2 + 2, reduce_dim, kernel_size=1, padding=0, bias=False),
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).to(device), s_y=torch.FloatTensor(1,1,473,473).to(device), s_seed=None, y=None):
        x_size = x.size()
...
########################### Context Module ###########################
        if self.pyramid:
            ...
            for idx, tmp_bin in enumerate(self.pyramid_bins):
                ...
                merge_feat_bin = torch.cat([guide_feat_bin, prob_map_bin, corr_mask_bin], 1)
        ...
        if self.training:
            ...
            aux_loss = torch.zeros_like(main_loss).to(device)
    ...
    def sp_center_iter(self, supp_feat, supp_mask, sp_init_center, n_iter):
        ...
        h_coords = torch.arange(h).view(h, 1).contiguous().repeat(1, w).unsqueeze(0).float().to(device)
        w_coords = torch.arange(w).repeat(h, 1).unsqueeze(0).float().to(device)
        ...
        sp_center = torch.zeros_like(sp_init_center).to(device)  # (C + xy) x num_sp
