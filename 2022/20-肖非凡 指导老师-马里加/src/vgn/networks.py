...
class ConvNet(nn.Module):
    ...
    def forward(self, x):   # x:[32,1,40,40,40]
        x = self.encoder(x)  #x[32,64,5,5,5]
        x = self.decoder(x)    #[32,16,40,40,40]
        qual_out = torch.sigmoid(self.conv_qual(x))  #[32,1,40,40,40]
        rot_out = F.normalize(self.conv_rot(x), dim=1)  #[32,4,40,40,40]
        width_out = self.conv_width(x)  #[32,1,40,40,40]
...
class Encoder(nn.Module):
    def __init__(self, in_channels, filters, kernels):
        ...
        self.resnet1= ResnetBlockFC(size_in= filters[0])
        self.resnet2= ResnetBlockFC(size_in= filters[1])
        self.resnet3= ResnetBlockFC(size_in= filters[2])
        

    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x)     #[32,16,20,20,20]
        x= self.resnet1(x)

        x = self.conv2(x)
        x = F.relu(x)     #[32,32,10,10,10]
        x= self.resnet2(x)

        x = self.conv3(x)
        x = F.relu(x)     #[32,64,5,5,5]
        x= self.resnet3(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):  
        x = self.conv1(x)  #[32,64,5,5,5]
        x = F.relu(x)       

        x = F.interpolate(x, 10)  #[32,64,10,10,10]
        x = self.conv2(x)
        x = F.relu(x)         #[32,32,10,10,10]

        x = F.interpolate(x, 20)  #[32,32,20,20,20]
        x = self.conv3(x)
        x = F.relu(x)        #[32,16,20,20,20]

        x = F.interpolate(x, 40)   #[32,16,40,40,40]
        return x
