...
def main(args):
    ...
    # 首先创建trainer engine，传入model,optimizer,criterion
    ...
    @trainer.on(Events.EPOCH_COMPLETED)  #当一个epoch结束之后会触发此事件（记录训练结果）
    ...
    @trainer.on(Events.EPOCH_COMPLETED) #当一个epoch结束之后会触发此事件（记录验证）
...
def create_train_val_loaders(root, batch_size, val_split, augment, kwargs):
    # load the dataset
    dataset = Dataset(root, augment=augment)  #grasp.csv
...
def prepare_batch(batch, device):
    tsdf, (label, rotations, width), index = batch  #tsdf:[32,1,40,40,40] label:[32] rotations[32,2,4] width:[32]  index:[32,3]
    tsdf = tsdf.to(device)
...
def select(out, index):
    qual_out, rot_out, width_out = out #[32,1,40,40,40]  [32,4,40,40,40]  [32,1,40,40,40]
    batch_index = torch.arange(qual_out.shape[0])   #[32]  [0,1,2,3,4,5...,31]
...
def create_trainer(net, optimizer, loss_fn, metrics, device):
    def _update(_, batch):
        ...
        # forward
        x, y, index = prepare_batch(batch, device)  #X: tsdf[32,1,40,40,40]  y:(label,r,w,) ([32],[32,2,4],[32])  index:[32,3]
        y_pred = select(net(x), index)
        loss, loss_dict = loss_fn(y_pred, y) #loss funciotn
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logdir", type=Path, default="/home/xff/VGN/GIGA/data/runs_pile")
    ...
    parser.add_argument("--augment", action="store_true")  #默认值为False
