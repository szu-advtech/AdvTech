...
import pointnet2_utils as pointnet2_utils
import pytorch_utils as pt_utils
from typing import List
import numpy as np
import time
import math
import Adaptive_Sampling  as AS
...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        for i in range(len(self.groupers)):
            ...
            # my code:
            # if self.npoint is not None:
            #     raw_group_xyz = new_features[:,0:3,:,:] # (B, 3, npoint, nsample)
            #     if self.first_layer:  
            #          group_features = new_features[:,3:6,:,:]
            #     else:      
            #         group_features = new_features[:,6:,:,:]
            #     adaptive_center, adaptive_center_features = self.adaptive_sample_layer(raw_group_xyz, group_features, self.nsamples[i]+1)
                
            #     new_adaptive_xyz = xyz_flipped.clone() # (B, C, N')
            #     b_idx = torch.arange(0,new_adaptive_xyz.shape[0], dtype=torch.long).view(-1,1).repeat(1,fps_idx.shape[1])
            #     xyz_mask = torch.zeros(new_adaptive_xyz.size(), dtype=torch.uint8).cuda()
            #     xyz_mask[b_idx,:,fps_idx.type(torch.cuda.LongTensor)] = 1 
            #     new_adaptive_xyz.masked_scatter_(xyz_mask, adaptive_center)
            #     #del xyz_mask
            #     if features is not None:
            #         new_adaptive_features = features.clone()
            #         feature_mask = torch.zeros(new_adaptive_features.size(), dtype=torch.uint8).cuda()
            #         feature_mask[b_idx,:,fps_idx.type(torch.cuda.LongTensor)] = 1
            #         new_adaptive_features.masked_scatter_(feature_mask, adaptive_center_features)
            #     #del feature_mask
            #     new_adaptive_features = self.groupers[i](new_adaptive_xyz, adaptive_center, new_adaptive_features, fps_idx)
            # new_features = self.mlps[i](new_adaptive_features)
...
class PointnetSAModuleMSG(_PointnetSAModuleBase):
    ...
    def __init__(
        ...
        as_in_channels = mlps[0][0]+3 if self.first_layer else mlps[0][0]
        as_out_channels = mlps[0][0]//2 if (mlps[0][0]//2) > 32 else 32
        self.adaptive_sample_layer = AS.AdaptiveSampling(as_in_channels, as_out_channels, [32, as_in_channels+1])
...
class PointnetFPModule(nn.Module):
    ...
    def __init__(self, *, mlp: List[int], bn: bool = True):
        super().__init__()
        self.mlp = pt_utils.SharedMLP(mlp, bn=bn, activation=nn.LeakyReLU(inplace=False))
