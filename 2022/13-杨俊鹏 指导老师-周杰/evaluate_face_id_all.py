...
def main():
    ...
    files = sorted(file_list, key=lambda x: os.path.getmtime(os.path.join(args.save_path, x)), reverse=True)
    best_total = 0
    best_model = None


    for file_name in files[0:80]:
        model.load_state_dict(torch.load(args.save_path + file_name))
        print('Load model successfully: %s' % (args.save_path + file_name))
        ...
        with torch.no_grad():
            ...
            print('Eval total_samples:{}'.format(Total_samples))
            acc = float(Correct / Total_samples)
            print('\n total acc: {:.6f}\n'.format(acc))
            if acc > best_total:
                best_total = acc
                best_model = file_name
            print("now best model: %s" % best_model)
            print("now best acc: %s" % best_total)
