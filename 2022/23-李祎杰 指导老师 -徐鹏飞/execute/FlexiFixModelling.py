...
def setConstraints(data: DataInstance, model: Model, relVars, boolVars, vVars, elemVars, posVars, N):
    ...
    # add Text width constraint
    for element in range(N):
        for other in range(N):
            if element > other:
                if (data.elements[element].elementType == "text" and data.elements[other].elementType == "text"):
                    #model.addConstr(W[element] == W[other], str(element)+" & "+str(other)+" width equal")
                    pass
    ...
    for element in range(N):
        for otherElement in range(N):
            if element > otherElement:
                #core MILP的第一个约束，for overlap & overflowing
                model.addConstr(
