...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, root, augment=False, return_pos=False):
        ...
        self.df = read_df(root)  # grasp.csv
    ...
    def __getitem__(self, i):
        ...
        ori = Rotation.from_quat(self.df.loc[i, "qx":"qw"].to_numpy(np.single))   #四元组初始化 [qx,qy,qz,qw]
        ...
        voxel_grid = read_voxel_grid(self.root, scene_id)  #return grid

        if self.augment:
            voxel_grid, ori, pos = apply_transform(voxel_grid, ori, pos)

        index = np.round(pos).astype(np.long)   #根据pos(x,y,z)确定体素的index
        rotations = np.empty((2, 4), dtype=np.single)
        R = Rotation.from_rotvec(np.pi * np.r_[0.0, 0.0, 1.0])  #旋转向量初始化  
        rotations[0] = ori.as_quat()   #表示为四元数
