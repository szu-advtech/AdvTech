...
if __name__ == '__main__':
    ...
    test(net1, testloader, device)
    inputs = torch.randn(1, 3, 32, 32).to(device)
    flops, params = get_flops.measure_model(net1, device, 3, 32, 32)
    print('Flops:', flops)
    print('Params', params)

    # 生成每层输出特征图秩的排序，用于后续的剪枝
    print('==> Get feature..')
    if args.get_feature:
        get_feature(net1, args.limit, device, trainloader)

    print('==> Prepare pruning..')
    # 裁剪后的模型
    ...
    # 进行裁剪并微调
    print('==> Hrank Pruning..')
    HRank(net1, prun_net, prun_optimizer, prun_scheduler, criterion, trainloader, device)

    # 裁剪后的正确率
    print('==> After Hrank pruning..')
    test(net1, testloader, device)
    inputs = torch.randn(1, 3, 32, 32).to(device)
    flops, params = get_flops.measure_model(net1, device, 3, 32, 32, 1-args.compress_rate)
    print('Flops:', flops)
    print('Params', params)

    # 进行裁剪并微调
    print('==> Random Pruning..')
    HRank(net2, random_net, anti_optimizer, anti_scheduler, criterion, trainloader, device)

    # 裁剪后的正确率
    print('==> After Random pruning..')
    test(net2, testloader, device)

    # 进行裁剪并微调
    print('==> Anti Hrank Pruning..')
    HRank(net3, anti_net, prun_optimizer, prun_scheduler, criterion, trainloader, device)

    # 裁剪后的正确率
    print('==> After Anti Hrank pruning..')
    test(net3, testloader, device)
