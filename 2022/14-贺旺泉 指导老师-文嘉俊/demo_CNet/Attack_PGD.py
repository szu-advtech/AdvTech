...
############### adversarial attack (PGD) ###############
        ...
        with torch.no_grad():
            output_ori, context_prior_map_ori = Model(images)
            output_ori = output_ori.data

        # Start iteration
        for i in range(args.num_iter):
            processed_image = processed_image.requires_grad_()

            # output_adv = Model(processed_image)
            output_adv, context_prior_map_adv = Model(processed_image)
            pred_loss = tpgd_loss(F.log_softmax(output_adv,dim=1), F.softmax(output_ori, dim=1))
            # seg_loss = criterion(output,label)
