...
def main(args):
    ...
    if '20ng' in input_dir:
        if 'contrastive' in options.model: torch.save(options, 'D:\Code\Python\CLNTM-master-update\weights\options\contrastive_20ng_50_options.pt')
        else: torch.save(options, r'D:\Code\Python\CLNTM-master-update\weights\options/20ng_50_options.pt')
    else:
        if 'contrastive' in options.model: torch.save(options, 'D:\Code\Python\CLNTM-master-update\weights\options\contrastive_imdb_50_options.pt')
        else: torch.save(options, r'D:\Code\Python\CLNTM-master-update\weights\options\imdb_50_options.pt')
    ...
    train_X, vocab, row_selector, train_ids, train_tfidf = load_word_counts(input_dir, options.train_prefix)
    ...
    train_tfidf, dev_tfidf = split_matrix(train_tfidf, train_indices, dev_indices)
    ...
    if options.test_prefix is not None:
        test_X, _, row_selector, test_ids, test_tfidf = load_word_counts(input_dir, options.test_prefix, vocab=vocab)
    ...
    model = train(model, network_architecture, train_X,train_tfidf, train_labels, train_prior_covars, train_topic_covars, training_epochs=options.epochs, batch_size=options.batch_size, rng=rng, X_dev=dev_X, Y_dev=dev_labels, PC_dev=dev_prior_covars, TC_dev=dev_topic_covars)
    ...
    if dev_X is not None:
        perplexity = evaluate_perplexity(model, dev_X, dev_tfidf, dev_labels, dev_prior_covars, dev_topic_covars, options.batch_size, eta_bn_prop=0.0)
    ...
    if test_X is not None:
        perplexity = evaluate_perplexity(model, test_X, test_tfidf, test_labels, test_prior_covars, test_topic_covars, options.batch_size, eta_bn_prop=0.0)
    ...
    if n_labels > 0:
        ...
        predict_labels_and_evaluate(model, train_X, train_tfidf, train_labels, train_prior_covars, train_topic_covars, options.output_dir, subset='train')

        if dev_X is not None:
            predict_labels_and_evaluate(model, dev_X, dev_tfidf, dev_labels, dev_prior_covars, dev_topic_covars, options.output_dir, subset='dev')

        if test_X is not None:
            predict_labels_and_evaluate(model, test_X, test_tfidf, test_labels, test_prior_covars, test_topic_covars, options.output_dir, subset='test')
    ...
    save_document_representations(model, train_X, train_tfidf, train_labels, train_prior_covars, train_topic_covars, train_ids, options.output_dir, 'train', batch_size=options.batch_size)

    if dev_X is not None:
        save_document_representations(model, dev_X, dev_tfidf, dev_labels, dev_prior_covars, dev_topic_covars, dev_ids, options.output_dir, 'dev', batch_size=options.batch_size)

    if n_test > 0:
        save_document_representations(model, test_X, test_tfidf, test_labels, test_prior_covars, test_topic_covars, test_ids, options.output_dir, 'test', batch_size=options.batch_size)
...
def load_word_counts(input_dir, input_prefix, vocab=None):
    ...
    # compute tdidf and load tdidf
    compute_tfidf.set_tfidf(input_dir, input_prefix)
    tfidf = compute_tfidf.get_tfidf(input_dir, input_prefix).tocsr()
    ...
    X = X[row_selector, :]
    tfidf = tfidf[row_selector, :]
    ids = [doc_id for i, doc_id in enumerate(ids) if row_selector[i]]

    return X, vocab, row_selector, ids, tfidf
...
def train(model, network_architecture, X, tfidf, Y, PC, TC, batch_size=200, training_epochs=100, display_step=10, X_dev=None, Y_dev=None, PC_dev=None, TC_dev=None, bn_anneal=True, init_eta_bn_prop=1.0, rng=None, min_weights_sq=1e-7):
    # Train the model
    n_train, vocab_size = X.shape

    mb_gen = create_minibatch(X, tfidf, Y, PC, TC, batch_size=batch_size, rng=rng)
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            # get a minibatch
            batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs = next(mb_gen)
            # do one minibatch update
            cost, recon_y, thetas, nl, kld = model.fit(batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci)
...
def create_minibatch(X, tfidf, Y, PC, TC, batch_size=200, rng=None):
    ...
    while True:
        ...
        tfidf_mb = np.array(tfidf[ixs, :].todense()).astype('float32')
        if Y is not None:
        ...
        yield X_mb, tfidf_mb, Y_mb, PC_mb, TC_mb


def get_minibatch(X, tfidf, Y, PC, TC, batch, batch_size=200):
    ...
    return X_mb, tfidf_mb, Y_mb, PC_mb, TC_mb


def predict_label_probs(model, X, tfidf, PC, TC, batch_size=200, eta_bn_prop=0.0):
    ...
    for i in range(n_batches):
        batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs = get_minibatch(X, tfidf, None, PC, TC, i, batch_size)
        Z, pred_probs = model.predict(batch_xs,batch_tfidf, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop)
...
def print_and_save_weights(options, model, vocab, prior_covar_names=None, topic_covar_names=None):
    ...
    if '20ng' in options.input_dir:
        if 'contrastive' in options.model.lower(): torch.save(model, 'D:/Code/Python/CLNTM-master-update/weights/contrastive_20ng_50.pt')
        else: torch.save(model, 'D:/Code/Python/CLNTM-master-update/weights/20ng_50.pt')
    else:
        if 'contrastive' in options.model.lower(): torch.save(model, 'D:/Code/Python/CLNTM-master/weights/contrastive_imdb_50.pt')
        else: torch.save(model, 'D:/Code/Python/CLNTM-master-updateweights/imdb_50.pt')
...
def evaluate_perplexity(model, X, tfidf, Y, PC, TC, batch_size, eta_bn_prop=0.0):
    ...
    n_batches = int(np.ceil(n_items / batch_size))
    for i in range(n_batches):
        batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs = get_minibatch(X, tfidf, Y, PC, TC, i, batch_size)
        batch_losses = model.get_losses(batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop)
...
def predict_labels_and_evaluate(model, X, tfidf, Y, PC, TC, output_dir=None, subset='train', batch_size=200):
    # Predict labels for all instances using the classifier network and evaluate the accuracy
    pred_probs = predict_label_probs(model, tfidf, X, PC, TC, batch_size, eta_bn_prop=0.0)
...
def save_document_representations(model, X, tfidf, Y, PC, TC, ids, output_dir, partition, batch_size=200):
    ...
    for i in range(n_batches):
        batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs = get_minibatch(X, tfidf, Y, PC, TC, i, batch_size)
        thetas.append(model.compute_theta(batch_xs, batch_tfidf, batch_ys, batch_pcs, batch_tcs))
