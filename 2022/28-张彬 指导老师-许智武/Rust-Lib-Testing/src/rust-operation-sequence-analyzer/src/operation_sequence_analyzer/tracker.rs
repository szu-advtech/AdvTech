...
impl<'a, 'b, 'c, 'tcx> Tracker<'a, 'b, 'c, 'tcx> {
    ...
    fn handle_init(&mut self, place: Place<'tcx>, contain_result: bool) -> Option<Place<'tcx>> {
        println!("local_kind: {:?} -> {:#?}", place, self.batch_depend_results.body.local_kind(place.local));
        ...
        let mut defs = rhses.iter().filter(|(_, result)| {
            *result == DependResult::CallDepend || *result == DependResult::MoveDepend || *result == DependResult::RefDepend || *result == DependResult::CopyDepend
        });
        println!("defs: {:#?}", defs);
