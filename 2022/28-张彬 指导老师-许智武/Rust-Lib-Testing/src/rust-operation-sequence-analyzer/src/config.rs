...
pub enum RustDetectorType {
    DoubleLockDetector,
    ConflictLockDetector,
    OperationSequenceDetector,
}

pub struct LockDetectorConfig {
    pub lock_detector_type: RustDetectorType,
    pub crate_name_lists: CrateNameLists,
}

impl LockDetectorConfig {
    pub fn from_env() -> Result<Self, &'static str> {
        let lock_detector_type = "RUST_DETECTOR_TYPE";
        let black_crate_name_lists = "RUST_DETECTOR_BLACK_LISTS";
        let white_crate_name_lists = "RUST_DETECTOR_WHITE_LISTS";
        let lock_detector_type = match env::var(lock_detector_type) {
            Ok(detector) => {
                if &detector == "OperationSequenceDetector" {
                    RustDetectorType::OperationSequenceDetector
                } else if &detector == "DoubleLockDetector" {
                    RustDetectorType::DoubleLockDetector
                } else if &detector == "ConflictLockDetector" {
                    RustDetectorType::ConflictLockDetector
                } else {
                    return Err("Env var \"RUST_DETECTOR_TYPE\" is not set or provided with wrong value.\nPlease set it to \"OperationSequenceDetector\", \"DoubleLockDetector\" or \"ConflictLockDetector\"");
                }
            },
            Err(_) => return Err("Env var \"RUST_DETECTOR_TYPE\" is not set or provided with wrong value.\nPlease set it to \"OperationSequenceDetector\", \"DoubleLockDetector\" or \"ConflictLockDetector\""),
        };
        ...
        if !black_crate_name_lists.is_empty() && !white_crate_name_lists.is_empty() {
            Err("Env var \"RUST_DETECTOR_BLACK_LISTS\" and \"RUST_DETECTOR_WHITE_LISTS\" are \nboth provided values. Please clear the values in one of them")
