...
mod operation_sequence_analyzer;

use config::*;
use conflict_lock_checker::ConflictLockChecker;
use double_lock_checker::DoubleLockChecker;
use operation_sequence_analyzer::OperationSequenceAnalyzer;
...
impl rustc_driver::Callbacks for DetectorCallbacks {
    fn after_analysis<'tcx>(
        ...
        println!("[ROSA] Entering rustc_driver!");
        queries.global_ctxt().unwrap().peek_mut().enter(|tcx| {
            let lock_config = LockDetectorConfig::from_env().unwrap();
            match lock_config.lock_detector_type {
                RustDetectorType::OperationSequenceDetector => match lock_config.crate_name_lists {
                    CrateNameLists::Black(crate_name_black_lists) => {
                        let mut operation_sequence_analyzer =
                            OperationSequenceAnalyzer::new(false, crate_name_black_lists);
                            operation_sequence_analyzer.check(tcx);
                    }
                    CrateNameLists::White(crate_name_white_lists) => {
                        let mut operation_sequence_analyzer =
                            OperationSequenceAnalyzer::new(true, crate_name_white_lists);
                            operation_sequence_analyzer.check(tcx);
                    }
                },
                RustDetectorType::DoubleLockDetector => match lock_config.crate_name_lists {
                    CrateNameLists::Black(crate_name_black_lists) => {
                ...
                RustDetectorType::ConflictLockDetector => match lock_config.crate_name_lists {
                    CrateNameLists::Black(crate_name_black_lists) => {
...
fn compile_time_sysroot() -> Option<String> {
    ...
    Some(match (home, toolchain) {
        ...
        _ => option_env!("RUST_SYSROOT")
            .expect("To build rust-operation-sequence-analyzer without rustup, set the `RUST_SYSROOT` env var at build time")
            .to_owned(),
    })
}
