...
use crate::fuzz_target::dataflow::{BatchDependResults, DependResult};
...
impl<'a, 'b, 'c, 'tcx> Tracker<'a, 'b, 'c, 'tcx> {
    ...
    pub fn track(&mut self) -> (Place<'tcx>, TrackerState) {
        loop {
            match self.state {
                ...
                TrackerState::Guard => {
                    println!("guard, place: {:?}", self.place);
                ...
                TrackerState::Result => {
                    println!("result place: {:?}", self.place);
                ...
                TrackerState::RefLock => {
                    println!("reflock, place: {:?}", self.place);
                ...
                TrackerState::WrapperLock => {
                    println!("wrapperlock, place: {:?}", self.place);
                ...
                TrackerState::LocalSrc => {
                    println!("localSrc, place: {:?}", self.place);
                    return (self.place, self.state);
                }
                TrackerState::ParamSrc => {
                    println!("paramSrc, place: {:?}", self.place);
                    return (self.place, self.state);
                }
                TrackerState::Option => {
                    println!("option, place: {:?}", self.place);
                    return (self.place, self.state);
                }
            }
        }
    ...
    fn handle_result(&mut self, place: Place<'tcx>) -> Option<Place<'tcx>> {
        ...
        // println!("result place: {:#?}", place);
        let rhses = self.batch_depend_results.get_depends(place);
        // println!("result rhses: {:#?}", rhses);
        let mut defs = rhses.iter().filter(|(_, result)| {
            true
            // *result == DependResult::CallDepend || *result == DependResult::MoveDepend
        });
        println!("result defs: {:#?}", defs);
        if defs.clone().count() > 1 {
            return None;
        }
        match defs.next() {
            ...
            Some((place, DependResult::MoveDepend))
            | Some((place, DependResult::RefDepend))
            | Some((place, DependResult::AddressOfDepend))
            | Some((place, DependResult::CopyDepend)) => {
                self.state = TrackerState::Result;
                Some(Place::from(place.local))
            }
