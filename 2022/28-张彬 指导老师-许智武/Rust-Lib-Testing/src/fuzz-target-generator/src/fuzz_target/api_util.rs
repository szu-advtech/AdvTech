...
pub fn get_generic_inner_name(ty: &clean::Type) -> Option<String> {
    //TODO：self不需要考虑，因为在产生api function的时候就已经完成转换，但需要考虑类型嵌套的情况
    match ty {
        clean::Type::Generic(name) => Some(name.as_str().to_string()),
        clean::Type::Tuple(types) => {
            // for ty_ in types {
            //     if _is_generic_type(ty_) {
            //         return true;
        ...
        //                 for generic_arg in args {
        //                     if let clean::GenericArg::Type(inner_ty) = generic_arg {
        //                         return get_generic_inner_name(inner_ty);
        //                     }
        //                 }
        //             }
        //             clean::GenericArgs::Parenthesized { inputs, output } => {
        //                 for input_ty in inputs {
        //                     return get_generic_inner_name(input_ty);
        //                 }
        //                 if let Some(output_ty) = output {
        //                     return get_generic_inner_name(output_ty);
        //                 }
        //             }
        //         }
        //     }
        ...
        | clean::Type::BorrowedRef { type_, .. } => {
            let inner_type = &**type_;
            return get_generic_inner_name(inner_type);
        }
        _ => {
            // 只考虑当前结构类型是否为泛型，因此Path的不考虑
            return None;
        }
    }
}
...
pub fn _type_name(type_: &clean::Type, full_name_map: &FullNameMap) -> String {
    if let Some(def_id) = &type_.def_id_no_primitives() {
        //&type_.def_id() {
        if let Some(full_name) = full_name_map.get_full_name(def_id) {
            return full_name.clone();
        }
    }
    match type_ {
        clean::Type::Primitive(primitive_type) => primitive_type.as_sym().as_str().to_string(),
...
pub fn _same_type(
    ...
    if hard_mode {
        _same_type_hard_mode(output_type, input_type, full_name_map, false)
    } else {
        //TODO:soft mode
...
pub fn _same_type_hard_mode(
    ...
    // println!("output: {:#?}\ninput: {:#?}", output_type, input_type);
    // 如果output和input有一个是generic，则为genericCall
    if output_type == input_type && (api_parameter::is_generic_type(output_type) || api_parameter::is_generic_type(input_type) || is_generic) {
        return CallType::_GenericCall;
    }
    ...
    if !api_parameter::is_generic_type(output_type) && api_parameter::is_generic_type(input_type) {
        ...
        let new_input_type = api_parameter::replace_generic_type(input_type, &inner_output_type);
        // println!("inner output: {:#?}", inner_output_type);
        // println!("new input: {:#?}", new_input_type);
        return _same_type_hard_mode(output_type, &new_input_type, full_name_map, true);
    }

    // input为确定类型
    if api_parameter::is_generic_type(output_type) && !api_parameter::is_generic_type(input_type) {
        let inner_input_type = api_parameter::get_inner_type(input_type);
        let new_output_type = api_parameter::replace_generic_type(output_type, &inner_input_type);
        // println!("inner input: {:#?}", inner_input_type);
        // println!("new output: {:#?}", new_output_type);
        return _same_type_hard_mode(&new_output_type, input_type, full_name_map, true);
    }
    ...
    match input_type {
        clean::Type::BorrowedRef {
            ...
            return _borrowed_ref_in_same_type(mutability, type_, output_type, full_name_map, is_generic);
        }
        clean::Type::RawPointer(mutability, type_) => {
            return _raw_pointer_in_same_type(mutability, type_, output_type, full_name_map, is_generic);
        }
        _ => {}
    }
    ...
    if prelude_type::_prelude_type_need_special_dealing(input_type, full_name_map) {
        ...
        let inner_call_type = _same_type_hard_mode(output_type, &final_type, full_name_map, is_generic);
        // print!("inner call type: {:#?}", inner_call_type);
    ...
    println!("match output type");
    match_output_type(output_type, input_type, full_name_map, is_generic)
}

pub fn match_output_type(output_type: &clean::Type, input_type: &clean::Type, full_name_map: &FullNameMap, is_generic: bool) -> CallType {
    //对输出类型进行分类讨论
    match output_type {
        //结构体、枚举、联合
        clean::Type::Path { .. } => {
            _same_type_resolved_path(output_type, input_type, full_name_map, is_generic)
        }
        //范型
        ...
        clean::Type::RawPointer(_, type_) => {
            _same_type_raw_pointer(type_, input_type, full_name_map, is_generic)
        }
        clean::Type::BorrowedRef { type_, .. } => {
            _same_type_borrowed_ref(type_, input_type, full_name_map, is_generic)
        }
        clean::Type::QPath { .. } => {
            //TODO:有需要的时候再考虑
            CallType::_NotCompatible
...
fn _same_type_resolved_path(
    ...
    if prelude_type::_prelude_type_need_special_dealing(output_type, full_name_map) {
        println!("prelude dealing for output");
        ...
        // println!("output prelude: {:#?}", output_prelude_type);
        // println!("final output: {:#?}", final_output_type);
        let inner_call_type = _same_type_hard_mode(&final_output_type, input_type, full_name_map, is_generic);
        println!("inner call type: {:#?}", inner_call_type);
        match inner_call_type {
            CallType::_NotCompatible => {
                return CallType::_NotCompatible;
            }
    ...
    match input_type {
        clean::Type::Path { .. } => {
            if *output_type == *input_type || _resolved_path_equal_without_lifetime(output_type, input_type){
    ...
    match primitive_type {
                        ...
                        | PrimitiveType::F64 => {
                            if output_primitive_type == input_primitive_type {
                                ...
                                return CallType::_AsConvert(
                                    input_primitive_type.as_sym().as_str().to_string(),
                                );
                            }
                        }
        ...
        PrimitiveType::Char => match input_type {
            clean::Type::Primitive(inner_primitive_type) => match inner_primitive_type {
                ...
                PrimitiveType::U8 => {
                    return CallType::_AsConvert(
                        inner_primitive_type.as_sym().as_str().to_string(),
                    );
                }
                _ => {
                    return CallType::_NotCompatible;
...
fn _same_type_raw_pointer(
    ...
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(inner_type, input_type, full_name_map, is_generic);
...
fn _same_type_borrowed_ref(
    ...
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(inner_type, input_type, full_name_map, is_generic);
    println!("inner compatible: {:#?}", inner_compatible);
    match inner_compatible {
        CallType::_NotCompatible => {
            return CallType::_NotCompatible;
        }
        _ => {
            // 可能涉及copy的类型，这里先不考虑，在generic_match的时候考虑
...
pub fn _borrowed_ref_in_same_type(
    ...
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(output_type, inner_type, full_name_map, is_generic);
    match &inner_compatible {
        CallType::_NotCompatible => {
            return CallType::_NotCompatible;
        }
...
pub fn is_fuzzable_type(ty_: &clean::Type, full_name_map: &FullNameMap) -> bool {
    let fuzzable = fuzzable_type::fuzzable_call_type_by_clean_type(ty_, full_name_map);
    match fuzzable {
        FuzzableCallType::NoFuzzable
        | FuzzableCallType::Generic => false,
        _ => true,
    }
}
...
pub fn _resolved_path_equal_without_lifetime(ltype: &clean::Type, rtype: &clean::Type) -> bool {
    if let clean::Type::Path {
        path: lpath,
        ..
    } = ltype
    {
        // is_generic:lis_generic, ..} = ltype {
        if let clean::Type::Path {
            path: rpath,
            ..
        } = rtype
        {
            //is_generic: ris_generic, ..} = rtype {
...
fn new_segments_without_lifetime(
    ...
    for old_path_segment in old_path_segments {
        ...
        if let clean::GenericArgs::AngleBracketed { args, bindings } = generic_args {
            ...
            for arg in args {
                match arg {
                    clean::GenericArg::Lifetime(..) | clean::GenericArg::Infer => {}
