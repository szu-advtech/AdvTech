...
impl CallType {
    pub fn _to_call_string(&self, variable_name: &String, full_name_map: &FullNameMap) -> String {
        match self {
            ...
            CallType::_ToString => {
                let mut call_string = variable_name.to_string();
                call_string.push_str(".to_string()");
                call_string
            }
            CallType::_Debug => {
                let mut use_string = String::from("println!(\"{:#?}\", ");
                use_string.push_str(variable_name);
                use_string.push_str(")");
                use_string
            }
            CallType::_Display => {
    ...
    pub fn _contains_move_call_type(&self) -> bool {
        match self {
            ...
            | CallType::_Debug
            | CallType::_Display 
            | CallType::_GenericCall => false,
            CallType::_UnwrapOption(..) | CallType::_UnwrapResult(..) => true,
            CallType::_BorrowedRef(call_type) | CallType::_MutBorrowedRef(call_type) => false, 
    ...
    pub fn _contains_unwrap_call_type(&self) -> bool {
        match self {
            ...
            | CallType::_Deref(call_type)
            | CallType::_ToOption(call_type)
            | CallType::_ToResult(call_type) => call_type._contains_unwrap_call_type(),
        }
    }
    ...
    fn _inner_array_to_call_type(call_type_array: &Vec<CallType>, start: usize) -> Self {
        ...
        match current_type {
            ...
            CallType::_GenericCall => CallType::_GenericCall,
        }
    }

    pub fn is_generic_call(&self) -> bool {
        println!("call type: {:#?}", self);
        match self {
            CallType::_DirectCall 
            | CallType::_AsConvert(..) 
            | CallType::_NotCompatible 
            ...
            CallType::_BorrowedRef(inner_type) => inner_type.is_generic_call(),
            CallType::_MutBorrowedRef(inner_type) => inner_type.is_generic_call(),
            CallType::_ConstRawPointer(inner_type, ref type_) => inner_type.is_generic_call(),
            CallType::_MutRawPointer(inner_type, ref type_) => inner_type.is_generic_call(),
            CallType::_UnsafeDeref(inner_type) => inner_type.is_generic_call(),
            CallType::_Deref(inner_type) => inner_type.is_generic_call(),
            CallType::_UnwrapOption(inner_type) => inner_type.is_generic_call(),
            CallType::_ToOption(inner_type) => inner_type.is_generic_call(),
            CallType::_UnwrapResult(inner_type) => inner_type.is_generic_call(),
            CallType::_ToResult(inner_type) => inner_type.is_generic_call(),
            CallType::_GenericCall => true,
        }
    }

    pub fn as_string(&self) -> String {
        match self {
            CallType::_DirectCall => String::from("DirectCall"),
            CallType::_GenericCall => String::from("GenericCall"),
            CallType::_AsConvert(..) => String::from("AsConvert"),
            CallType::_NotCompatible => String::from("UNCALLABLE"),
            CallType::_ToString => String::from("ToString"),
            CallType::_Debug => String::from("Debug"),
            CallType::_Display => String::from("Display"),
            CallType::_BorrowedRef(inner_type) => format!("ImmutBorrowedRef->{}", inner_type.as_string()),
            CallType::_MutBorrowedRef(inner_type) => format!("MutBorrowedRef->{}", inner_type.as_string()),
            CallType::_ConstRawPointer(inner_type, ref type_) => format!("ConstRawPtr->{}", inner_type.as_string()),
            CallType::_MutRawPointer(inner_type, ref type_) => format!("MutRawPtr->{}", inner_type.as_string()),
            CallType::_UnsafeDeref(inner_type) => format!("UnsafeDeref->{}", inner_type.as_string()),
            CallType::_Deref(inner_type) => format!("Deref->{}", inner_type.as_string()),
            CallType::_UnwrapOption(inner_type) => format!("BorrowedRef->{}", inner_type.as_string()),
            CallType::_ToOption(inner_type) => format!("ToOption->{}", inner_type.as_string()),
            CallType::_UnwrapResult(inner_type) => format!("UnwrapResult->{}", inner_type.as_string()),
            CallType::_ToResult(inner_type) => format!("ToResult->{}", inner_type.as_string()),
        }
    }
}
