            ...
            match api_type {
                ...
                ApiType::UseParam => {
                    ...
                    for param in &api_call.params {
                        one_sequence.push_str(format!("use {}", index).as_str());
                        one_sequence.push_str(" ");
                    }
                }
            }
...
pub fn _print_pretty_dependencies(graph: &ApiGraph) {
    ...
    for dependency in &graph.api_dependencies {
        let (output_type, output_index) = &dependency.0.output_fun;
        let (input_type, input_index) = &dependency.0.input_fun;
        ...
        match output_type {
            ...
            ApiType::ControlStart | ApiType::ScopeEnd => {}
            ApiType::UseParam => {
        ...
        res.push_str(" ");
        res.push_str(dependency.0.input_param_index.to_string().as_str());
        res.push_str(" ");
        print!("{:?}", res);
        println!("{:?}", dependency.0.call_type);
        //res.push_str("\r\n");
    }
}
