...
def infer_cam_mp(process_id, image_ids, label_list, cur_gpu):
    ...
    for i, (img_id, label) in enumerate(zip(image_ids, label_list)):
        ...
        if args.out_la_crf is not None:
            crf_la = _crf_with_alpha(org_img,cam_dict, args.low_alpha,t=10)
            np.save(os.path.join(args.out_la_crf, img_id + '.npy'), crf_la)

        if args.out_ha_crf is not None:
            crf_ha = _crf_with_alpha(org_img,cam_dict, args.high_alpha,t=10)
            np.save(os.path.join(args.out_ha_crf, img_id + '.npy'), crf_ha)
