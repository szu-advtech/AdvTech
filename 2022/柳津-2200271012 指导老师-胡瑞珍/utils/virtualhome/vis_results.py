...
class VIS_Compare(VIS_BASE):
    ...
    def set_render(self, *args, **kwargs):
        ...
        if 'room_bbox' in kwargs['type']:
            ...
            size_=np.diag(np.array(self.room_bbox['size']) / 2.)
            mat_=self.room_bbox['R_mat']
            vectors = np.diag(np.array(self.room_bbox['size']) / 2.).dot(self.room_bbox['R_mat'])
            color = [125, 125, 125]
            self.box_actor = self.get_bbox_cube_actor(centroid, vectors, color, 0.05)
            self.box_actor.GetProperty().SetInterpolationToPBR()
            renderer.AddActor(self.box_actor)
        ...
        if 'scene_geo' in kwargs['type']:
            self.ply_actor = self.set_actor(self.set_mapper(self.set_ply_property(self.scene_geo), 'model'))
            self.ply_actor.GetProperty().SetOpacity(1)
            self.ply_actor.GetProperty().SetInterpolationToPBR()
            renderer.AddActor(self.ply_actor)
...
def parse_args():
    ...
    parser.add_argument('--room-id', type=int, default=3,
                        help='Give a scene id in [0-N].')
    parser.add_argument('--script-id', type=int, default=201,
...
if __name__ == '__main__':
    ...
    if args.pred_or_gt == 'gt':
        ...
        render_window_interactor=viser.visualize(type=vis_args, save_path=str(save_path), offline=False, mode='pred')
        render_window_interactor.Initialize()

        cb = vtkTimerCallback()
        cb.actor = viser.box_actor
        render_window_interactor.AddObserver('TimerEvent', cb.execute)
        cb1 = vtkTimerCallback()
        cb1.actor = viser.ply_actor

        render_window_interactor.AddObserver('TimerEvent', cb1.execute)
