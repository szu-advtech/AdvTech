...
class cosine_similarity_loss(nn.Module):
  ...
  def forward(self,x, y):
    output = F.cosine_similarity(x, y, dim=1).mean()
    return 1-output
...
def creat_model(cfg, args):
    ...
    area_criterion = AreaLoss().to(device)
    print('Preparing networks done!')
    return device, model, optimizer, cls_criterion, area_criterion
...
def main():
    ...
    device, model, optimizer, cls_criterion, area_criterion = creat_model(cfg, args)

    best_gtknown = 0
    best_top1_loc = 0
    best_top1_loc_eopch=1
    update_train_step = 0
    update_val_step = 0
    for epoch in range(1, cfg.SOLVER.NUM_EPOCHS+1):
        ...
        update_train_step, loss_train, cls_top1_train, cls_top5_train = \
            train_one_epoch(train_loader, model, device, cls_criterion, area_criterion,
        ...
        print("Best TOP1_LOC_Epoch: {}".format(best_top1_loc_eopch))
        print(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M'))


def train_one_epoch(train_loader, model, device, criterion, area_criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        cls_logits, bg_activation = model(input, return_cam=False)
        loss = criterion(cls_logits, target) + area_criterion(bg_activation)
