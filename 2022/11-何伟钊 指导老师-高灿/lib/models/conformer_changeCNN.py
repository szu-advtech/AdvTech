...
__all__ = [
    'conformer_tscam_small_patch16_224']
class Mlp(nn.Module):
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., vis=False):
        ...
        self.vis = vis
        self.conv3x3 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=False)
    def forward(self, x):
        B, N, C = x.shape
        ...
        n, p, c = x_patch.shape
        x_patch = torch.reshape(x_patch, [n, int(p ** 0.5), int(p ** 0.5), c]).permute(0,3,1,2)
        n, c, h, w = x_patch.shape
        bg = 1 - c2p.reshape(n, 1, h, w)
        x_patch = (x_patch * bg).permute(0,2,3,1).reshape(n, p, c)
...
class Conformer(nn.Module):
    ...
    def forward(self, x, return_cam=False):
        ...
        n, c, h, w = x_patch.shape
        # bg_activation = 1 - attn_weights.sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1)
        x_patch = self.trans_cls_head(x_patch)
        x_logits = self.pooling(x_patch).flatten(1) + conv_cls  # patch_cls + conv_cls
        ...
        if self.training:
            ...
            # feature_map = x_conv.detach().clone()
            cams = attn_weights.sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1)
            attn_maps = attn_weights.sum(0)[:, 1:, 1:]
            # cams = torch.matmul(cams.reshape(n, 1, -1), attn_maps).reshape(n,1,h,w)
            cams = cams * feature_map                           # B * C * 14 * 14
            return x_logits, feature_map
