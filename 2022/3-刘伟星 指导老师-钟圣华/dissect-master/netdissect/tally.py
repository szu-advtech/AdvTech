...
def tally_quantile(compute, dataset,fake_path,sample_size=None,batch_size=1,
    ...
    #quantile_transforms = transforms.Compose([transforms.ToTensor()])
    #dataset = torchvision.datasets.ImageFolder(fake_path,transform=quantile_transforms)

    loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True)
    rq = runningstats.RunningQuantile()
    img_index = 0
    for batch in pbar(loader):
        sample = call_compute(compute, batch,img_index)
        rq.add(sample)
...
def call_compute(compute,batch,img_index):
    '''Utility for passing a dataloader batch to a compute function.'''
    if isinstance(batch, list):
        return compute(img_index,*batch)
