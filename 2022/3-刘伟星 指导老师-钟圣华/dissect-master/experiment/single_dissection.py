...
def parseargs():
    ...
    aa('--dissect_units', type=int, default=50)
...
def acquire_activation(args,path,model,dataset,layername,level_at_99,sample_size,batch_size):
    if not os.path.exists(path+'/acts.pth'):
        loader = tally.make_loader(dataset, sample_size=sample_size, batch_size=batch_size)
        acts=torch.zeros(sample_size,512,16,16)
        act_index=0
        for batch in loader:
            data_batch = batch[0].cuda()
            fake_images = model(data_batch)
        ...
        torch.save(acts,path+'/acts.pth')
    else:
        acts=torch.load(path+'/acts.pth')
    return acts

def acquire_dsscore(path,acts,masks,sample_size):
    if not os.path.exists(path + '/ds_score.pth'):
        ...
        for unit in range(acts.shape[1]):#对每个unit操作
            ...
            for sample_index in range(sample_size):
                ...
                union=acts[sample_index,unit]+masks[sample_index][0]-isect
                iou=isect/union
                iou = torch.where(torch.isnan(iou), torch.full_like(iou, 0), iou).sum()
                iou_each_unit+=iou
            ds_score[unit]=iou_each_unit/sample_size
        torch.save(ds_score,path +'ds_score.pth')
    else:
        ds_score=torch.load(path+'/ds_score.pth')
    return ds_score

def dissection(model,dissect_unit,layername,dataset,path,sample_size,batch_size):
    def zero_some_units(layer,x, *args):
        #x[:, dissect_unit] = 0
        x[:, dissect_unit] = 0
        return x
    def dsscore_partial(x,*args):
        #x[:,]
        pass
    loader = tally.make_loader(dataset, sample_size=sample_size, batch_size=batch_size)
    index=0
...
def main():
    ...
    resdir = 'results/%s-%s-%s-quantile%s-units%s' % (
            ...
            int(args.quantile * 1000),int(args.dissect_units))
    def resfile(f):
...
##############               准备模型、数据集、参数                   ########################
    ...
    dissect_units=args.dissect_units
###############           保存原始模型的生成图片                   ############################
    # with torch.no_grad():
...
###############            计算对应层的激活值、并上采样              ###########################
    pbar.descnext('rq')
    def compute_samples(img_index,batch, *args):
...
################   获取threshold后的activation map，     #################################
    acts=acquire_activation(args,resfile(''),model,dataset, layername,level_at_99, sample_size,batch_size)


##################    计算DS    ################################
    ds_score=acquire_dsscore(resfile(''),acts,masks,sample_size)


#################################   计算最高激活map    ######################################
    sort_score,indices=torch.sort(ds_score)
    dissect_unit=indices[-dissect_units:]

######################    消融  保存生成图片  ###############################
    dissection(model, dissect_unit, layername, dataset, resfile(''), sample_size, batch_size)
