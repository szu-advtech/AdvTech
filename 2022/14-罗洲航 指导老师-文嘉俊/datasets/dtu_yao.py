...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
        ...
        self.mode = mode
        self.nviews = nviews                    #多视点总数(实现中取3=1ref+2src)
        self.ndepths = ndepths                  #深度假设数(默认假设192种不同的深度)
        self.interval_scale = interval_scale    #深度间隔缩放因子
    ...
    def read_cam_file(self, filename):     #相机外参、相机内参、最小深度(都为425)、深度假设间隔(都为2.5)
    ...
    def __getitem__(self, idx):
        ...
        return {"imgs": imgs,                     #3个3channel的512*640大小的图片
                "proj_matrices": proj_matrices,
                "depth": depth,                   #ref的深度图 (128, 160)
                "depth_values": depth_values,     #ref将来要假设的所有深度值
                "mask": mask}                     #ref深度图的mask
