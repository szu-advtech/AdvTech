...
parser.add_argument('--pre_loadckpt', default=None, help='load a specific checkpoint')
...
def save_depth():
    ...
    # load checkpoint file specified by args.pre_loadckpt
    print("loading model {}".format(args.pre_loadckpt))
    state_dict = torch.load(args.pre_loadckpt)
    ...
    print('Number of model parameters after prunning: {}'.format(sum([p.data.nelement() for p in model.parameters()])))
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"], search=False)
