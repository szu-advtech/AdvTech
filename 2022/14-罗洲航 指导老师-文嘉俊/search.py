...
parser.add_argument('--train_epochs', type=int, default=2, help='number of epochs to train')
parser.add_argument('--search_epochs', type=int, default=1, help='number of epochs to search')
...
parser.add_argument('--cost_priority', default=True, help='continue to train the model')

parser.add_argument('--summary_freq', type=int, default=20, help='print and summary frequency')
parser.add_argument('--save_freq', type=int, default=100, help='save checkpoint frequency')
...
def train():
    ...
    # searching
    for epoch_idx in range(start_epoch, args.search_epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs, image_outputs = train_sample(sample, detailed_summary=do_summary, search=True)
            if do_summary:
                save_scalars(logger, 'train', scalar_outputs, global_step)
                save_images(logger, 'train', image_outputs, global_step)
            del scalar_outputs, image_outputs
            print(
                'Epoch {}/{}, Iter {}/{}, train loss = {:.3f}, time = {:.3f}'.format(epoch_idx+1, args.search_epochs, batch_idx+1,
...
def train_sample(sample, detailed_summary=False, search=True):
    ...
    if args.cost_priority:
        for name, layer in model.named_modules():
            if 'layer' in name and isinstance(layer, nn.BatchNorm3d):
                layer.weight.grad.data.add_(lamda * beta[int(name[-6])] * torch.sign(layer.weight.data))
