...
"""
        The function takes in an environment and a configuration dictionary, and then initializes the agent's value network,
        target network, loss function, and optimizer

        :param env: The environment that the agent will be interacting with
        :param config: The configuration dictionary
        
        该函数接收环境和配置字典，然后初始化代理的价值网络，
        目标网络、损失函数和优化器
        ：param env：代理将与之交互的环境
        ：param config:配置字典
        """
...
class DQNAgent(AbstractDQNAgent):
    def __init__(self, env, config=None):
        ...
        # Creating a model based on the configuration.
        # 基于配置创建模型。
    ...
    def step_optimizer(self, loss):
        """
        > The optimizer takes the loss function and backpropagates it through the network to update the weights

        :param loss: The loss function that we're trying to minimize
        """
        """
        >优化器获取损失函数并通过网络反向传播，以更新权重
        ：param loss：我们试图最小化的损失函数
        """
        # Optimize the model
    ...
    def compute_bellman_residual(self, batch, target_state_action_value=None):
        """
        We take the state-action values from the value network, and the target state-action values from the target network,
        and compute the loss

        :param batch: a batch of transitions
        :param target_state_action_value: The target state-action value. This is the value that the agent is trying to
        predict
        :return: The loss, the target state action value, and the batch.
        """
        """
        我们从价值网络获取状态动作值，从目标网络获取目标状态动作值，
        并计算损失
        ：param batch：一批转换
        ：param target_state_action_value：目标状态操作值。这是代理尝试的值
        预测
        ：return：损失、目标状态操作值和批次。
        """
    ...
    def get_batch_state_values(self, states):
        """
        > Given a batch of states, return the value of each state and the action that would be taken in that state

        :param states: a list of states
        :return: The values and actions are being returned.
        """
    ...
    def get_batch_state_action_values(self, states):
        """
        It takes a batch of states and returns the corresponding batch of state-action values

        :param states: a list of states
        :return: The value of the state-action pair.
        它接受一批状态并返回相应的一批状态-动作值:param states:状态列表
        :return:状态-动作对的值。
        """
        # Taking a batch of states and returning the corresponding batch of state-action values.
        # 获取一批状态并返回相应的一批状态操作值。
